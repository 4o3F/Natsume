...
pub(super) const FUTURE_ROUTE: &str = "/api/v2/device/control";
pub(super) const FUTURE_SUBPROTOCOL: &str = "natsume.control.v2";
const TRANSCRIPT_ROUTE: &[u8] = FUTURE_ROUTE.as_bytes();
const TRANSCRIPT_SUBPROTOCOL: &[u8] = FUTURE_SUBPROTOCOL.as_bytes();
...
const EXPECTED_CLIENT_INIT_SHA256: [u8; 32] = [
    0x8a, 0x40, 0x75, 0x16, 0x1d, 0x2e, 0xa8, 0xac, 0x9d, 0x02, 0x15, 0xb6, 0xaa, 0x58, 0xb7, 0x4f,
    0x85, 0x3f, 0xec, 0xd4, 0x47, 0x92, 0x92, 0xe0, 0x9f, 0xbc, 0x66, 0x22, 0xdf, 0x0b, 0x83, 0x6a,
];
const EXPECTED_TRANSCRIPT_SHA256: [u8; 32] = [
    0xe8, 0x42, 0x97, 0x80, 0xf6, 0x4c, 0xa3, 0xee, 0x81, 0xd1, 0x67, 0xe6, 0x5c, 0x90, 0xcf, 0x02,
    0xd9, 0x8f, 0xff, 0x82, 0xf5, 0x1c, 0x09, 0xcc, 0x07, 0xc1, 0xc7, 0x8f, 0x8b, 0x8d, 0x01, 0x9b,
];
const EXPECTED_SIGNATURE: [u8; 64] = [
    0xf2, 0xc2, 0x3d, 0xb9, 0x59, 0xdc, 0x99, 0xb0, 0x21, 0x53, 0x5d, 0x44, 0x85, 0xc4, 0x61, 0x27,
    0x45, 0x3c, 0xba, 0xba, 0x85, 0x3e, 0x94, 0x0d, 0x9d, 0xca, 0x26, 0xb3, 0x89, 0x02, 0x16, 0xa7,
    0x1c, 0x8d, 0x22, 0x7c, 0x06, 0x62, 0xbd, 0xd1, 0xf6, 0x6d, 0x8b, 0x92, 0x82, 0x0c, 0x41, 0xb8,
    0xc3, 0xc3, 0xa9, 0x04, 0x7c, 0xc8, 0x4c, 0xd3, 0xcb, 0x36, 0x39, 0xfd, 0x48, 0x7c, 0x38, 0x04,
];
...
pub(super) fn assert_deterministic_vectors() {
    ...
    assert_eq!(sha256(&init_bytes), EXPECTED_CLIENT_INIT_SHA256);
    let proof = sign_proof(&key, challenge, &init, EXPECTED_CLIENT_INIT_SHA256);
    let transcript = proof_transcript(challenge, &proof);
    assert_eq!(transcript.len(), 255);
    assert_eq!(sha256(&transcript), EXPECTED_TRANSCRIPT_SHA256);
