...
Push a tag such as `v2.3.2` to run [release.yml](../.github/workflows/release.yml).
The workflow accepts `vMAJOR.MINOR.PATCH` and SemVer prerelease suffixes such as
`v2.3.2-rc.1` (no build metadata). It reuses the full CI workflow at the tagged
commit; all jobs must pass before GitHub Release publication. Branch/PR CI keeps
its `2.3.2~ci1` package version.
...
release notes. If `docs/releases/<tag>.md` exists at the tagged commit, its
release and migration guidance is included before the generated notes. For
prereleases, `v2.3.2-rc.1` becomes Debian version `2.3.2~rc.1`,
which sorts before `2.3.2`; the GitHub Release is marked as a prerelease and is
