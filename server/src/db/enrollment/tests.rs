...
use crate::{
    application::{
        ...
        enrollment::{
            self, EnrollmentError, EnrollmentOutcome, EnrollmentRequestId, EnrollmentRequestInput,
            GatewayIssuer, HardwareIdentityQuality, TEST_CONTEST_END, TEST_GATEWAY_HOSTNAME,
...
#[tokio::test]
async fn live_request_capacity_rejects_the_next_intake_without_writes() -> Result<(), TestFailure> {
    let fixture = DatabaseFixture::new();
    let database = fixture.connect().await?;
    provisioning::open_window(&database, correlation_id())
        .await
...
fn assert_exact_gateway_leaf_profile(leaf_der: &[u8]) -> Result<ParsedLeafProfile, TestFailure> {
    ...
    if !exact_site_san
        ...
        || !exact_key_usage
        || certificate.validity().not_after.timestamp() != site.gateway_not_after().unix_seconds()
    {
...
fn expect_pending(result: Result<EnrollmentOutcome, EnrollmentError>) -> Result<Uuid, TestFailure> {
    match result.map_err(|_| TestFailure::IntakeFailed)? {
...
async fn assert_reactivation_issuance(
    ...
    let (actor, reason, encoded_detail) = audit_shape(database, "issue_device_credentials").await?;
    let detail: Value =
