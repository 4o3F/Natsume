...
use crate::{
    application::operator::{OperatorError, OperatorIdentity, SessionFacts},
    db::Transaction,
};
...
fn find_session_persisted(
    ...
    row.map(|row| {
        if row.expiry_valid != 1 || !matches!(row.expired, 0 | 1) {
            ...
            .read(|transaction| {
                diesel::sql_query(
                    "EXPLAIN QUERY PLAN \
                     ...
                     CASE WHEN julianday(sessions.expires_at) IS NULL THEN 0 ELSE 1 END, \
                     CASE WHEN julianday(sessions.expires_at) <= julianday('now') THEN 1 ELSE 0 END \
                     FROM operator_sessions AS sessions \
