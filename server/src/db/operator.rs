...
use crate::{
    application::operator::{
        AccountFacts, OperatorError, OperatorIdentity, OperatorRole, SessionCredentialHash,
        SessionFacts,
    },
...
fn create_first_admin_in_transaction(
...
fn expire_session_in_transaction(
    connection: &mut SqliteConnection,
    credential_hash: &[u8],
    correlation_id: CorrelationId,
) -> Result<Option<SessionFacts>, OperatorStoreError> {
...
fn session_facts(row: SessionRow) -> SessionFacts {
    SessionFacts {
...
fn terminate_session_in_transaction(
    connection: &mut SqliteConnection,
    credential_hash: &[u8],
    correlation_id: CorrelationId,
    audit_event_id: AuditEventId,
...
pub(crate) mod tests {
    ...
    pub(crate) async fn test_business_counts(
        database: &Database,
    ...
    #[tokio::test]
    async fn first_admin_is_persisted_with_frozen_hash_and_redacted_audit()
    -> Result<(), TestFailure> {
        ...
        let parsed =
            PasswordHash::new(&account.3).map_err(|_| TestFailure::PersistedPhcWasInvalid)?;
        ...
        assert_database_excludes_canaries(
            &fixture.database,
            &[login_name, password, &password_hash],
        )
        .await?;
        Ok(())
    }
    ...
    -> Result<(), TestFailure> {
        let fixture = TestDatabase::new().await?;
        let identity =
            insert_operator(&fixture.database, "session-admin", OperatorRole::Admin).await?;
        let raw = [0x3c_u8; 32];
    ...
    async fn session_reads_do_not_renew_and_expiry_is_lazy_once() -> Result<(), TestFailure> {
        let fixture = TestDatabase::new().await?;
        let identity =
            insert_operator(&fixture.database, "expiry-admin", OperatorRole::Admin).await?;
    ...
    async fn termination_is_repeat_safe_and_concurrent_once() -> Result<(), TestFailure> {
        ...
        if after_termination.sessions != 0
            || after_termination.audits != before_termination.audits + 1
        {
    ...
    async fn assert_termination_noops(
        ...
        for value in noops {
            terminate_application_session(
                database,
                correlation_id(),
                SessionCredentialHex::new(value),
            )
            .await
            .map_err(|_| TestFailure::SessionTerminationFailed)?;
        }
    ...
    async fn session_audit_failures_roll_back_creation_and_termination() -> Result<(), TestFailure>
    {
        let fixture = TestDatabase::new().await?;
        let identity =
            insert_operator(&fixture.database, "atomic-admin", OperatorRole::Admin).await?;
        let duplicate_audit_id = AuditEventId::from_uuid(Uuid::now_v7());
        insert_existing_audit(&fixture.database, duplicate_audit_id).await?;
    ...
    async fn insert_existing_audit(
        ...
        database
            .interact(move |connection| {
                let event = AuditEvent::first_admin_created(
                    audit_event_id,
                    correlation_id(),
                    Uuid::now_v7(),
                );
