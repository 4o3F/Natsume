...
#[tokio::test]
async fn provisioning_window_role_matrix_and_current_fact_read_are_exact() -> Result<(), TestFailure>
{
...
fn request(method: Method, path: &str, cookie: Option<&str>) -> Result<Request<Body>, TestFailure> {
...
fn assert_window_response(
    ...
    let value: Value =
        serde_json::from_slice(&response.body).map_err(|_| TestFailure::WindowResponseChanged)?;
    let object = value
        .as_object()
