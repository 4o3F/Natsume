...
use super::super::super::{
    ...
    tests::{
        ...
        cookie_request, drive, header_text, login_request, normalized_error_response_body, request,
        response_body_text, response_contains, seed_operator, unused_vault_master_key,
        unused_web_root,
    },
};
...
fn test_router(database: &Database) -> axum::Router {
    router(
        database.clone(),
...
#[tokio::test]
async fn request_rejections_and_invalid_cookies_are_uniform_and_redacted() -> Result<(), TestFailure>
{
    let fixture = TestDatabase::new().await?;
    let application = test_router(&fixture.database);
...
-> Result<(), TestFailure> {
    ...
    let remaining_sessions = db_operator::tests::test_session_and_audit_counts(&reopened)
        ...
        .0;
    let retry_application = router(reopened, unused_vault_master_key(), unused_web_root());
...
async fn tls_request(
    ...
    let mut stream = connect_test_client(address, identity.ca_certificate(), IpAddr::V4(LOCALHOST))
        .await
        .map_err(|_| TestFailure::TlsSessionRequestFailed)?;
    stream
        .write_all(request)
