...
pub(crate) async fn sign_in(
    ...
    let account = database
        .read(move |transaction| db::operator::find_account(transaction, &login_name))
        .await?;
...
pub(crate) async fn create_session_with_audit_id(
    ...
    database
        .write(move |transaction| {
            db::operator::insert_session(transaction, &credential_hash, identity)?;
...
pub(crate) async fn authenticate_session(
    ...
    let Some(facts) = database
        .read(move |transaction| db::operator::query::find_session(transaction, &snapshot_hash))
        .await?
    else {
    ...
    let cleanup = database
        .write(move |transaction| {
            let Some(current) = db::operator::query::find_session(transaction, &cleanup_hash)?
            else {
            ...
            let deleted = db::operator::delete_session_by_hash(transaction, &cleanup_hash)?;
            if deleted != 1 {
...
pub(crate) async fn terminate_session_with_audit_id(
    ...
    database
        .write(move |transaction| {
            let Some(current) = db::operator::query::find_session(transaction, &credential_hash)?
            else {
                return Ok(());
            };
            let deleted = db::operator::delete_session_by_hash(transaction, &credential_hash)?;
            if deleted != 1 {
                return Err(OperatorError::PersistenceFailed);
            }
