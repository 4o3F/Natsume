...
2. The seven category enums and unified `ErrorCode` contain exactly the 34 normative public semantics.
...
The crate is deliberately value-only. It has no Axum, Prost, zbus, UUID, regex, or domain-error dependency. Owning adapters integrate the 34-code registry with generated contracts without deriving behavior from error text.
...
3. extend the 34-code registry contract test and its exhaustive stable-string match;
...
Run locked workspace formatting, build, tests, Clippy, and the repository policy scan. Registry tests prove the exact 34-code catalog, category conversion, stable Serde values, unknown-code rejection, and exhaustive stable-string mapping. When real adapter mappings exist, their owning crates must provide exhaustive domain-to-stable and stable-to-transport behavior tests.
