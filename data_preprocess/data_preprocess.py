...
def generate_teams_json(
    df: pl.DataFrame, org_mapping: Dict[str, str]
) -> List[Dict[str, Any]]:
...
def generate_accounts_yaml(df: pl.DataFrame) -> List[Dict[str, Any]]:
    ...
    for row in df.iter_rows(named=True):
        account_data = {
            "id": str(row["account"]),
            "username": str(row["account"]),
            "password": str(row["password"]),
            "type": "team",
            "team_id": str(row["account"]),
...
def main():
    ...
    try:
        ...
        print(
            "organizations.json, teams.json and accounts.yaml generated successfully."
        )
