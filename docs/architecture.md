...
| Unified Device WSS（ADR-0038 目标） | server-auth TLS + connection-local Challenge proof；Enrollment 另经窗口与人工审核 | Enrollment/Resume、Command、Observed、binding | 101 后可分类终止最多发一次 Handshake/Active 共用 `ServerClose{error_code,ServerCloseAction}` 后关闭；审核拒绝单独走 durable `EnrollmentReviewStatus(DENIED)` |
