...
**2026-08-25 协议重基线**：Server 以 Device 内 durable `enqueue_order` 投递，Client executor 仍按 channel 严格串行；disable/revoke 在 eviction 前收口旧 Device 非终态 Command。`HOME_RESET` 只有在 completion record crash-safe 持久化后才可报 success/推进 Observed；无法安全构造 status/Observed 时保持 BLOCKED 并发送无 action 的 `ClientClose`。当前 Rust/migration 尚未同步。
