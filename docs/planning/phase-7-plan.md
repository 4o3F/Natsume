...
- runbook 至少覆盖 10 个流程：bootstrap 首启（TTY）、`reset-operator-password`、provisioning 窗口开关、Enrollment 审批/拒绝、revoked Device 的 destructive reprovision（全量清除旧 identity-bound material、全新 key/CSR/`enrollment_id`、人工审核后创建新 `device_id`，旧历史保留）、CSV import commit/discard、`SYNC_STATE`/`SYNC_SECRET` 触发、Home reset（operator-present）、备份/恢复、单生命周期竞赛重置（破坏性，删除 pending candidate 后下一次 import 走普通 first-import lifecycle）。
