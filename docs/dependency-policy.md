...
- **consumer/owner**：`natsume-device-protocol` 拥有 Ed25519 `public_key` identity（无 ControlKeyId）、proof transcript、canonical ClientProof digest 与 `verify_strict`；`natsume-device-daemon` 仅拥有 daemon-local PKCS#8 control-key lifecycle 与 manifest pin hex(public_key)；isolated ordinary-WSS feasibility 与 protocol contract tests 复用公开向量。Server 不直接调用 Dalek，而是经 protocol crate 使用受审查的 strict verifier。
- **问题**：为单一原位 Device control 协议提供专用 Ed25519 identity、connection-local Challenge/Proof、canonical proof digest 与 crash-safe本地 key foundation；当前 Token/Bearer authority 在 flag day 前仍保持，dormant key 不参与认证。
- **dependency/features**：workspace 准入 `ed25519-dalek 2.2`，关闭 defaults，只启用 `alloc`、`pkcs8`、`zeroize`；key/nonce entropy 继续使用 workspace `getrandom 0.4`，不启用第二套 RNG stack。`prost`、`prost-types` 与 `prost-build` 精确固定 `0.14.4`，因为 control-proof 语义摘要以该版本对已校验 typed Challenge/Proof 的确定性 length-delimited encoding 为规范；升级必须重算 golden 并复审 decode/merge 语义。
