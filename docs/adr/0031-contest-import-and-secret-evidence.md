...
**2026-08-24 修订（Binding/Account credential context）**：`device_bindings` 以 `seat_id` 为 PK（FK `seats`）、`device_id` NOT NULL UNIQUE（FK `devices`），并保存 `binding_id` TEXT NOT NULL UNIQUE——每次 bind 铸造的 canonical lowercase hyphenated UUIDv7 occupancy stamp，独立于 Seat。unbind 删除行，再次 bind 得到新 UUID。`SyncState.bound` 冻结 `binding_id` 与 mapping 当前 `account_id`；`SyncSecret` 与 Observed credential 以 `(binding_id,account_id,credential_revision)` 为完整上下文。`BindingResult` 不携带 occupancy `binding_id`。Device 在当前 assignment 缺失或 binding/Account 不同时拒绝 secret。Import 仍零 Binding 写入，mapping 改变会使旧 credential context 失效。
