...
模块内 SNAFU 与稳定的本地已知 ErrorCode catalog 必须作为一个完整架构被接受：本地 typed failure 保留 ownership，public boundary 提供穷举映射、稳定字符串和默认脱敏。跨版本 Protobuf 接收边界则不能把本地 catalog 当作 wire 闭集；未知但语法合法的 token 必须保持 opaque。
...
- 独立 `natsume-error-code` registry 拥有本构建可产生和可解释的 closed known-code catalog 与稳定字符串；不得依赖 Axum、Prost、zbus、具体持久化实现或具体 domain module。这个闭集只约束本地生成与 typed 分支，不把 Protobuf wire token 变成闭集。
- 依赖方向固定为：
...
- 同一公开语义在所有 transport 使用同一 `ErrorCode`。HTTP 显式映射 status 与 error response body/correlation ID；Protobuf/CommandStatus 使用稳定字符串；D-Bus 使用显式稳定 error name。
- Protobuf 的 `error_code` 是非空、不超过 64 ASCII bytes 且匹配 `[A-Z][A-Z0-9_]*` 的开放 token。未知但合法的 token 必须连同 envelope 的显式 state/action 一并接受并保持 opaque；不得把“本地 catalog 未知”升级为 protocol violation，也不得用它驱动 domain behavior、retry/auth policy、UI raw text 或 metric label。语法非法的 token 才是边界校验失败。
...
- 旧 flat implementation-specific codes 在形成 published compatibility baseline 之前即被分类 registry 替换；这是 coordinated pre-release baseline reset，不是删除已发布 compatibility contract。该 active-development window 内的 code 新增同样不需要 compatibility plan，目标 catalog 不复制手工计数，权威清单只见[契约](../contracts.md) §12。Rust registry 与既有 Phase 0 evidence 仍是较早快照，须在 flag-day implementation batch 同步，不能冒充当前协议证据。catalog 一旦成为 published compatibility baseline，任何删除或语义变更仍必须有 compatibility plan，本处置不放宽上一条规则。
- public Client 对本地已知 `ErrorCode` 只能按已审查映射和明确安全的 typed fields 分支，不得解析 title、detail、Display、source-chain 或翻译文本；未知 Protobuf token 不产生分支，只服从伴随的 typed state/action。
...
- Web、Device、Agent、Helper 和所有 transport 对已知 code 共享稳定语义，同时允许新 peer 的未知合法 Protobuf token 安全穿越旧接收方。
...
- 新 public code 是长期承诺，不能因实现方便随意添加；开放 wire token 也要求接收方显式区分 known/unknown。

## Acceptance basis and revisit trigger

正式接受该架构不表示所有 phase 已完成。证据必须覆盖 module-local SNAFU、所有 public adapter 的 exhaustive mapping、registry uniqueness、HTTP/Protobuf/D-Bus 显式表示、Protobuf ErrorCode 的 64-byte ASCII grammar 与 unknown-opaque handling、`COMMAND_ID_INVALID` 的 canonical UUIDv7 正/反例、`COMMAND_REQUEST_CONFLICT` 的 same-ID/different-fingerprint `409`、same request `200` replay、generated contract clean diff，以及 password/token/key/CSR/path/source-chain canary 的负向 redaction；Web/Device 不得解析 Display，未知 Protobuf token 不得驱动行为、UI 或 metrics。
