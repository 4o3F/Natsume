...
> Scope: Device control identity, unified Enrollment/control WSS, dynamic Machine authority actors and Device lifecycle lanes, credential activation, and destructive pre-release cutover
> Consolidates: —
...
**2026-08-25 修订（Machine aggregate、Client 双槽与 canonical generation）**：HWID-sharded runtime owner 明确为 `MachineActor`，其内部以 `DeviceLane(device_id)` 隔离每次 Device lifecycle 的 lease、Observed、Command FIFO 与 eviction；revoked predecessor 与 successor 不共享 lane-local state。Client durable local state 由可并存的 `current_authority` 与 `candidate_enrollment` 两个槽位组成，candidate 存在时始终以 exact Enrollment purpose 恢复，Activated 的本地提交原子替换 current authority 并清除 candidate。固定 identity 配方的 Enrollment aggregate quality 只有 `MEDIUM` / `STRONG`。任何参与 canonical proof/bundle/state hash 的 typed message 字段集合变化均视为 wire-generation breaking change，必须切换 WS subprotocol。
...
Exact WS subprotocol 是 control wire generation 的唯一版本权威。`ServerChallenge` / `ClientProof` 不再携带或回显 numeric protocol version；不支持 `natsume.control.v1` 的 peer 在 101 前协商失败。未来 breaking wire 使用新 subprotocol，不在同一 schema 内再建立一套 version negotiation。由于本协议会对 semantic decode 后重新编码的 `ServerChallenge` / `ClientProof`、`CredentialBundle` 与 `SyncState` 做 proof 或 hash，**这些 canonical root 及其递归可达 nested message 的字段集合发生任何变化——包括通常被视为 additive 的 optional 字段——都属于 generation-breaking change**；旧 peer 会丢弃 unknown field，不能与新 peer 得到同一 canonical bytes。

`ClientProof.purpose` 是结构化 `oneof`，不是可被服务端改写的 hint。缺失 purpose 拒绝。冻结的 purpose：

| Purpose | 签名 key | 结构化内容 |
|---|---|---|
| `EnrollmentAttempt` | 候选新 key | 必带 canonical UUIDv7 `enrollment_id`、32-byte `candidate_public_key`、非空 `gateway_csr_der` 与 `EnrollmentEvidenceQuality::{MEDIUM,STRONG}` aggregate `evidence_quality` |
| `ResumeSession` | 当前 active key | 空 message；不得携带任何 Enrollment material |

Device 在第一次网络尝试前 crash-safe 持久化 `enrollment_id`、control key、Gateway key、exact CSR 与当次计算的 `evidence_quality`。本地不是单一互斥 phase，而是两个正交 durable slots：`current_authority: Option<ActiveManifest>` 保存当前已激活 authority，`candidate_enrollment: Option<Prepared | BundleInstalled>` 保存正在审核或激活的 exact transaction。candidate slot 存在时，它自己的连接每次都从持久化 material 重建完全相同的 `EnrollmentAttempt`，不得因后续启动时 anchor 可用性变化而重新计算 quality；current-authority slot 的连接只发送 `ResumeSession`。两槽可在 replacement/recovery 期间并存，但 proof purpose 对**每条连接**仍严格互斥。Server 不把 Enrollment 改写为 Resume，也不在拒绝后要求 Device 猜测另一 purpose。control-key rotation 不是 handshake。
...
`EnrollmentAttempt.evidence_quality` 不是 UI 或网络层任意打分，而是 Device 本地 identity 配方对已通过 2-of-3 claim 的 present anchors 按固有 quality 排序后取第二高值：它表示形成 quorum 的最低质量；三个 slot 全部 present 时仍取第二高值。当前固定 anchors 为 `strong + strong + medium`，所以 wire `EnrollmentEvidenceQuality` 只有 `STRONG` 与 `MEDIUM` 两个可达值；不存在 `WEAK`。`strong + strong`（含第三个 medium）为 `STRONG`，`strong + medium` 为 `MEDIUM`。该值在首次 attempt 前随 transaction material 固化并由 candidate key 签名，但 Server 不接收 raw anchors，因而不能独立复算或证明该质量。Panel 必须标为 Device self-reported advisory evidence；它不放宽 2-of-3、placeholder、unsupported 或 startup strict-equality 规则，也不授予 authority。
...
### Unified dynamic MachineActor and Device lifecycle lanes

Registry 启动为空，不枚举 persisted Device。只有在 proof、bounded DB classification 与 capacity admission 成功后，才按需创建或复用 process-lifetime `MachineActor`。

Machine Hardware ID 是 primary runtime shard，只负责让 first Enrollment、existing reconnect 与 DeviceId operator action 汇聚同一个 `MachineActor`，不授予 authority。`MachineActor` 是物理机器 aggregate；其内部每个持久化 `device_id` 对应独立 `DeviceLane`。DeviceId 与 `public_key` aliases 必须同时解析到 MachineActor 与准确 lane/transaction，alias presence 同样不授予 authority。

MachineActor 是 durable Enrollment transaction、operator review、immutable CredentialBundle delivery、CredentialAck、EnrollmentActivated/Ready barrier、Resume、lifecycle、Command dispatch/status、Observed 与 disconnect 的唯一排序点。每个 MachineActor 只有一个有界 mailbox；所有 Device-targeted operator action 与 socket event 以 actor 接受顺序串行决策。消息在进入业务处理前必须解析出准确 `device_id` lane 或 candidate transaction；不得把“同 HWID”当作可跨 lifecycle 共享 lease、Observed、Command 或 eviction authority。
...
Operator approve/reject 只向 actor 当时的 current attachment 发送 Bundle/Denied；无 attachment 或 send 失败不回滚 durable transaction，下次 exact replay 依当前状态收敛。Candidate Handshake attachment 属 MachineActor 的 transaction slot；旧 Active lease 属准确的 current DeviceLane。Credential replacement/recovery 期间二者独立并存：替换 candidate attachment 不驱逐旧 Active lease，只有 exact Ack activation transaction 才 supersede 旧 key/lease。

每个 non-revoked DeviceLane 同时至多一个 current control lease；HWID 的单 non-revoked 约束使一个 MachineActor 同时至多一个可建立 authority 的 lane。认证与 durable barrier 通过后，actor 生成一个 UUIDv7，以 RFC 9562 network-order 16 bytes 写入 `SessionReady.session_id`；该 lease 在 actor 内必须携带 owner `device_id`，但 wire 仍只回显 `session_id`。lease 不写数据库，也不属于公开 Identifier 目录。actor 必须先在准确 lane 原子安装新 lease、使该 lane 旧 lease 的后续 Active frame 全部失效，再发送新连接的 `SessionReady`；若旧 socket 仍可写，则 best-effort 发送 `ServerClose{error_code=CONTROL_CONNECTION_SUPERSEDED,action=STOP}` 后关闭，其 Active outer envelope 回显被终止的旧 `session_id`。authority 转移不依赖旧 socket 是否收到 Close。由同一连接 exact `EnrollmentReady` replay 得到原 SessionReady/lease；任何重连都生成新 UUIDv7 lease。
...
Ack transaction commit 后，Server authority 已 durable active，但连接尚未进入 Active phase。Server 发送 `EnrollmentActivated{enrollment_id, device_id, bundle_sha256}`；Client 验证 transaction/hash，在一个 crash-safe local transaction 中以这些 facts 新建或替换 `current_authority`，并清除 exact `candidate_enrollment`，之后才能发送 `EnrollmentReady{enrollment_id, device_id,bundle_sha256}`。该提交失败不得 Ready；可安全编码时发送一次 `ClientClose` 后关闭。对于 replacement，提交前旧 Active manifest 与 BundleInstalled candidate 可以同时存在；若 Server Ack 已提交而 Client 在本地提交前崩溃，candidate slot 仍然存在并在重启时优先重放 exact `EnrollmentAttempt`，不得因旧 manifest 尚在而改发旧 `ResumeSession`。`EnrollmentReady` 只是本连接的 durable-client barrier，不是新的 Server durable state。Server 收到完全匹配的 Ready 后在准确 DeviceLane 安装新 current control lease 并发送 `SessionReady`；同一连接重复的 exact Ready 重放同一 SessionReady。若 Ready 或 SessionReady 丢失并断线，Client 已持有新 Active manifest 且 candidate 已清除，下一条连接直接使用 `ResumeSession` 获取新的 UUIDv7 lease。
...
| `active` / candidate 仍为 Prepared 或 BundleInstalled；可同时保留旧 Active manifest | candidate connection 用 exact `EnrollmentAttempt`；旧 authority connection 只可尝试旧 `ResumeSession` | 重放 exact Bundle；exact Ack no-op；旧 key 若已 superseded 则旧 Resume 拒绝 | Server 重放 Activated，Client 原子替换 current authority 并清除 candidate |
| candidate 已清除且 Client 已持久化当前 Active；Ready 或 SessionReady 丢失 | `ResumeSession` | 校验 active key/Device 后创建新 lease | 新连接收到 SessionReady |

**2026-08-23 修订（可恢复 Enrollment transaction；2026-08-25 双槽澄清）**：下列图冻结 Client 本地权威、candidate transaction、Proof purpose 与 control `enrollment_requests.state` 的转移。`current_authority` 与 `candidate_enrollment` 是两个正交槽位，不是一个四态 enum。Prepared 与 BundleInstalled candidate 均未 enrolled；Server transaction active 与 Client 新 manifest durable 是两个明确的 barrier。`SessionReady` 只携带本连接的 16-byte UUIDv7 `session_id`；Active 帧只回该值做 current-lease 校验。Active envelope 不携带 `authority_revision`。
...
本地权威与 Enrollment transaction 是不同事实。先落下 identity、canonical UUIDv7 `enrollment_id`、create-only control key、Gateway key 与 exact CSR，才能创建 candidate 并签 Enrollment Proof；Gateway 证书只存在于 Server 下发的 CredentialBundle 中，Client 在 Ack 前 crash-safe 落盘。首次 Enrollment 时 `current_authority` 为空；replacement/recovery 时旧 `current_authority` 可以与 candidate 并存。candidate connection 在整个 Prepared/BundleInstalled 恢复期只使用 Enrollment purpose，current-authority connection 只使用 Resume purpose。Client 只有在收到 `EnrollmentActivated` 并原子提交“替换 current authority + 清除 candidate”后，才让新 key 使用 Resume purpose。

```mermaid
stateDiagram-v2
    state current_authority {
        [*] --> NoCurrentAuthority
        NoCurrentAuthority --> ActiveManifest: first Activated local commit
        ActiveManifest --> ActiveManifest: ResumeSession obtains a new lease
        ActiveManifest --> ActiveManifest: replacement Activated atomically replaces manifest
    }
    state candidate_enrollment {
        [*] --> NoCandidate
        NoCandidate --> Prepared: persist enrollment_id, keys and exact CSR
        Prepared --> Prepared: exact EnrollmentAttempt retry
        Prepared --> BundleInstalled: persist exact CredentialBundle
        BundleInstalled --> BundleInstalled: exact EnrollmentAttempt retry and re-Ack
        BundleInstalled --> NoCandidate: Activated local commit clears candidate
    }
```

`Prepared` / `BundleInstalled` candidate 本身均未 enrolled；authority 只来自独立 current slot。缺 ID/key/CSR、坏 key/CSR、identity 不匹配或任一槽位 material 不一致全部 fail closed，不自动生成新 transaction 或把 candidate 改发 Resume。candidate 存在时，即使 Server transaction 已 active、旧 Active manifest 仍存在，Device 仍用相同 EnrollmentAttempt 取回 exact Bundle，经幂等 Ack 取回 `EnrollmentActivated`。control-key rotation 不是 handshake。
...
`SessionReady` 只表示认证连接与准确 DeviceLane 的 current lease 已建立。该 lane 随后处于 `AwaitingInitialObserved`：Client 的第一条 Active envelope 必须是 fresh `ObservedStateSnapshot`。MachineActor 在完成语义校验并把 snapshot 持久化到 lease owner lane 前不投递 Command，也不受理 BindingRequest 等其他业务包。可安全分类的非法首包、无效 Observed 或 Server 持久化失败以 `ServerClose` 终止；Client 若因本地 current-fact 损坏而无法安全构造真实 Observed，则保持数据面 BLOCKED、best-effort 发送 `ClientClose` 并关闭，不能伪造 `ABSENT`。每次重连重新执行该 barrier；SQLite 中上一条 Observed 只是 last-known，不能代替当前 lane 的 fresh barrier。无 `observed_sequence`、`observed_session_id` 或额外 ACK。

Server MachineActor 以单消费者有界 channel 接收 operator action、socket event 与 lifecycle event；每条事件先解析到准确 DeviceLane。Client WS reader 把 Command 放入单消费者有界 command channel；唯一 executor 严格按出队顺序执行。每个 lane 同时最多投递一条尚未终态的 Command，收到终态后才投递下一条。WS I/O、ping/pong、Observed 与 disconnect 检测可并发，但业务 Command 的本地副作用不并发。不增加 wire sequence 或 generation。

Command 的确定顺序由数据库内每 Device 严格递增的 `enqueue_order` 冻结，不由时间戳、UUID 或 actor 重启后的 mailbox 到达次序猜测。它是 `1..=i64::MAX` 的内部字段，`UNIQUE(device_id,enqueue_order)`，不进 wire。所有首次创建（包括 Device 离线）都路由到 HWID 的 MachineActor，再进入目标 `DeviceLane(device_id)`；lane 首次装载读取该 `device_id` 已持久化最大值，在同一串行 turn 分配下一值并原子提交 Command + audit。Exact HTTP replay/conflict 不分配。Crash gap 合法，不要求连续；到达 SQLite INTEGER 上界 fail closed。Lane 重建时若存在自己的 `in_flight`，先按 cut/initial Observed 规则收敛它；否则按 `enqueue_order` 选择最早 queued，因而进程重启不改变执行顺序。revoked predecessor 与 successor 的 order allocator、in-flight 与 queued 集合永不混用。

disable/revoke lifecycle event 与 Command status 在同一 MachineActor 中、按准确 DeviceLane 排序，并由 guarded DB transaction 最终仲裁。Lifecycle transaction 在提交 Device/key/certificate/audit 变化与 post-commit eviction 前，将该旧 `device_id` lane 全部 `queued` Command（Converge/Oneshot）置为 `failed/COMMAND_NOT_DELIVERED`，全部 `in_flight` 置为 `outcome_unknown`，既有终态不变。前者可确定零 wire 副作用；后者可能已有副作用，而 lifecycle 已切断未来 Observed 收敛。若 status 先提交就保留该终态；若 lifecycle 先提交，旧 lease 的迟到 status 无 authority。Post-commit evictor 只在 current lease owner `device_id` 精确匹配 lifecycle target 时关闭该 lease；对 predecessor 的 repeat noop 不能影响 successor。终态 Command rows 保留，HTTP exact replay/conflict 不变；未来 lifecycle 也不得复活旧意图。
...
后续还必须证明 capacity、manual-review/issuance/Ack crash cuts、Bundle/Active-manifest write-before-Ack/Ready、Client current-authority/candidate 双槽与 replacement Ack 后崩溃恢复、单 current Handshake attachment 的 exact-replay replacement 及其与旧 Active lease 并存、MachineActor/DeviceLane 路由隔离、lane-local `enqueue_order` 分配/重启恢复、owner-tagged single current UUIDv7 lease replacement 与 predecessor-noop 不驱逐 successor、initial-Observed barrier、immutable replay、revoked predecessor 的新 Device reprovision/旧 Resume 永久拒绝、exact `natsume.control.v1` 的 sole-version authority 与 canonical-message field-set breaking rule、Handshake/Active `ServerCloseAction`、无 action 的 `ClientClose`、Denied 与 Close 的终止分流、ErrorCode 64-byte ASCII grammar/unknown-opaque handling、canonical SemVer、`EnrollmentEvidenceQuality` 的 MEDIUM/STRONG 值域及首次持久化/exact replay/self-reported Panel 标识、state/error-code 矩阵、required/non-empty bounded UTF-8 `SyncSecret.password`、disable/revoke Command 终态化、revision/epoch 上界、Gateway state/hash 独立组合、filesystem durability、lifecycle ordering 与 500–600 Device envelope。Foundation 落地不关闭 G4。
