...
>
> **2026-08-25 修订（durable FIFO 与 lifecycle 收口）**：Command row 新增 Device 内严格递增的 `enqueue_order`；DeviceActor 对所有首次创建串行分配，重启从数据库恢复。disable/revoke 在驱逐连接前把 queued 收口为 `failed/COMMAND_NOT_DELIVERED`、in-flight 收口为 `outcome_unknown`。Device 只有在本地结果 crash-safe 持久化后才能发送 success status/完成 observation。
...
- Server DeviceActor 与 Client command executor 各有一个单消费者有界 channel。所有首次创建（含离线 target）都进入该 DeviceActor；actor 装载时读取已持久化最大 `enqueue_order`，在同一串行 turn 分配下一值并原子提交 Command + audit。Exact replay/conflict 不消费序号；crash gap 合法，上界 fail closed。actor 重建先收敛唯一 `in_flight`，否则按 `enqueue_order` 投递最早 queued。Server 同时最多一条 `in_flight` Command；Client 严格按出队顺序完成本地副作用与 terminal status，然后才开始下一条。WS I/O、ping/pong、Observed 与 disconnect 检测不被长 Command 阻塞。无 wire sequence、generation 或 Device-wide clock。
...
- `SYNC_STATE` 只应用 non-secret Target，绝不签发、携带或安装 certificate/token。Account mapping 变化即使旧 credential context 失效；Device 先应用新 non-secret assignment 并保持 BLOCKED，直到同 `(binding_id,account_id,credential_revision)` 的 `SecretBytes` 与完整 context 原子、crash-safe 安装。只有安装 fact durable 后才能发送 `CommandStatus(SUCCEEDED)` 或 Observed `INSTALLED`；失败保留旧 secret/明确 BLOCKED，无法安全构造 FAILED status 时以 `ClientClose` 结束连接。每个 Command 的 frozen content 只在 typed `frozen_payload_json` 中表示。
