...
**当前 authority**继续使用公开 HTTPS Enrollment、Device Token 与 Bearer-before-101。Production Proto 已按预发布 BC 原位拆为六个文件，单一 package 为 `natsume.device.control`，目标双端 subprotocol 由唯一版本权威 `natsume.control.v1` 选择。当前 Token Rust runtime 尚使用旧 token，待 authority flag day 同步。Handshake envelopes 为 `ServerHandshakeEnvelope`（`ServerChallenge` | `EnrollmentReviewStatus` | `CredentialBundle` | `EnrollmentActivated` | `SessionReady` | `ServerClose`）与 `ClientHandshakeEnvelope`（`ClientProof` | `CredentialAck` | `EnrollmentReady` | `ClientClose`）；Active envelopes 为 `ClientActiveEnvelope` / `ServerActiveEnvelope`，前者允许 `ClientClose`、后者允许 `ServerClose`。无 `ClientInit`、无 `ControlEnvelope`、无 Hello。Migration 已包含 transitional control-key/bundle columns，但 Token application 不读写它们。
...
**2026-08-24 修订（Command 串行与 Observed slim）**：七种 Command 不是同一套 Device journal 耐久机。Server MachineActor 的准确 DeviceLane 与 Client executor 各用有界 single-consumer channel，每 lane 最多一个 in-flight。Converge（`sync_state` / `sync_secret` / `reset_home`）按完整领域键幂等，仅在同一非终态 operator 意图仍 drift 时重推同一 ID/payload；Oneshot（`lock_session` / `unlock_session` / `terminate_session` / `open_binding_prompt`）仅 live socket，未投递明确失败、进入 delivery-attempt cut 后结果丢失为 Server-only `outcome_unknown`，均不重放。`PUT /api/v2/commands/{command_id}` 仍作 operator 审计；正确性是 payload 幂等 / live delivery，不是 Client journal。无 Heartbeat protobuf；keep-alive 为 WS ping/pong。Observed 为 slim snapshot（`applied_hash`、完整 `CredentialObservation{binding_id,account_id,credential_revision,state}`、`gateway_state`、`gateway_leaf_sha256`、`session_state`、可选 `completed_home_epoch`）。无 ControlKeyId：Server natural key 是 `public_key`，daemon manifest pins hex(public_key)。Enrollment/WSS Token 路径仍在按 [ADR-0038](adr/0038-unified-ordinary-wss-device-control-authority.md) 重写。

**2026-08-24 修订（OPEN_BINDING_PROMPT 单 in-flight）**：`OPEN_BINDING_PROMPT` 是 Oneshot，空 body，无 TTL、无 `prompt_message_id`。Device 打开 binding-prompt screen 即报 `CommandStatus` `SUCCEEDED`。现场确认绑定不是该 Command 的成功：Device 发起 `BindingRequest{seat_code}` → Server `BindingResult{state,error_code}`。每 active session 同时最多一个 prompt/request，以连接内顺序关联，无 `binding_request_id` 或 capability token。`CommandState` 只有 `SUCCEEDED` | `FAILED`；`error_code` 是受限开放 ErrorCode token，不是 `stable_error_code`。
...
**2026-08-24 修订（单控制租约与字段域）**：每台 Device 同时至多一个 current control lease；新认证连接以 Server-generated 16-byte UUIDv7 `session_id` 原子替换旧 lease。持久化 revision/epoch 的 protobuf 值统一限制在 `1..=i64::MAX`。`ClientProof` 两个版本字段冻结为 canonical SemVer；aggregate `evidence_quality` 是 present hardware anchors 的第二高质量，首次 attempt 前持久化且 exact replay，不随重启重算。它是 candidate-signed、Server 不独立验证 anchors 的 Device self-report，仅供人工审核与 Panel 展示。

**2026-08-24 修订（durable delivery cut）**：`commands.state` 的非终态统一为 `queued` / `in_flight`。Server 在任何 socket write 前 durable 提交 `queued → in_flight`；Oneshot 在此前失去所属 live lease 为 `failed/COMMAND_NOT_DELIVERED`，此后无 terminal status 为 `outcome_unknown`。Converge 的 `in_flight` 由新连接 initial Observed 推定成功、退回 `queued` 重投，或 fail closed。Command 不使用 deadline；当前 schema 残留列待 flag day 删除。

**2026-08-24 修订（单 wire 版本、Handshake attachment 与开放 ErrorCode）**：删除 Challenge/Proof numeric `protocol_version`，exact `natsume.control.v1` 是唯一 control wire generation 权威。每个非终态 Enrollment transaction 同时至多一个 current Handshake attachment，新 exact replay 替换旧 attachment 而不提前驱逐 replacement 期间的旧 Active lease。ErrorCode 是受限 ASCII token；未知但语法合法的 token 必须作 opaque 值接受，不驱动行为、不直接展示或作 metrics label。

**2026-08-25 修订（Client terminal report、durable queue 与 terminal lifecycle）**：Handshake/Active Client envelope 统一允许 `ClientClose{error_code}`；它只报告本连接无法安全继续，不带 retry action，不替代 Ack、CommandStatus 或任何 durable transaction transition。`commands.enqueue_order` 冻结为 Device 内持久化顺序，所有首次创建经目标 DeviceLane 串行分配。disable/revoke 事务在驱逐前把该旧 Device lane 的非终态 Command 收口；revoked 身份永不恢复，显式本地 reprovision 只能经新的人工审核 Enrollment 创建新 `device_id`。

**2026-08-25 修订（Machine/Device lane、Client 双槽与 Proto 可达域）**：动态 runtime owner 是 HWID-sharded `MachineActor`，内部以 `DeviceLane(device_id)` 隔离 lease、Observed、Command FIFO 和 lifecycle eviction；旧 revoked lane 的 noop 不能驱逐 successor lane。Client durable state 由可并存的 `current_authority` 与 `candidate_enrollment` 两槽组成，replacement Ack 后崩溃时 candidate exact replay 优先于旧 manifest Resume。Enrollment aggregate enum 收紧为只有 `MEDIUM` / `STRONG` 的 `EnrollmentEvidenceQuality`；`SyncSecret.password` message 必须 present，值继承非空、UTF-8、control-free、最多 512 bytes 的 Account password 契约。参与 canonical proof/bundle/state hash 的 typed message 任何字段集合变化都必须切换 WS subprotocol。

**已接受目标**由 [ADR-0038](adr/0038-unified-ordinary-wss-device-control-authority.md) 定义：普通 WSS 内的 Challenge/Proof、统一人工审核且不过期的 durable Enrollment transaction、`EnrollmentReviewStatus`、`CredentialBundle`/`CredentialAck`、`EnrollmentActivated`/`EnrollmentReady` barrier、`SessionReady{session_id = 16-byte UUIDv7}`、双向 terminal close、initial Observed barrier、Ed25519 control key 与动态 MachineActor/DeviceLane。`ClientProof` 使用结构化 `oneof purpose`：`EnrollmentAttempt` 必带 durable `enrollment_id`、candidate public key、exact CSR 与首次 attempt 前一并持久化的 aggregate `EnrollmentEvidenceQuality`；`ResumeSession` 为空且只供 current-authority slot 使用。对应 Proto foundation 已存在，但尚无 runtime authority consumer。
...
**2026-08-24 修订（`binding_id` occupancy UUID）**：`device_bindings.binding_id` 是每次 bind 铸造的 canonical lowercase hyphenated UUIDv7 occupancy stamp（`TEXT NOT NULL UNIQUE`），独立于 Seat；unbind 删除行，再次 bind 得到新 UUID。无 integer bump，无 `seats.binding_generation`，无 `revision_counters`。目标 Observed 以完整 `CredentialObservation` 表示安装上下文。`SyncState.bound.binding_id` 与 `SyncSecret.binding_id` 是 occupancy string UUIDv7；`SyncState` 的 unbound/bound 由 `oneof` 区分。`BindingRequest` / `BindingResult` 都不携带 occupancy `binding_id`；Device 从随后的显式 `SYNC_STATE` / `SYNC_SECRET` 得知该 stamp。Device 在当前 Binding row、Account mapping 或相应 ID/revision 不同时拒绝 Command。`GET /api/v2/bindings` row 含 `binding_id`。Import 仍零 Binding 写入。
...
> **ADR-0038 flag-day 边界**：本小节是“当前生产写入器”注册表，不是目标 Enrollment/Command 状态机。Atomic cutover 时必须同批删除 `expire_enrollment_requests`、`same_spki_retry`、Enrollment `window_closed` expiry 等旧词汇，并先注册 manual admission/review/approve+issue/deny/Ack-activation 与目标三终态的 typed audit 形状。disable/revoke lifecycle audit 还必须新增 `queued_command_failed_count` 与 `in_flight_command_unknown_count` 两个 redacted count；它不为每个被收口的 Command 伪造 `device:control`/`device_reported` audit，具体 ID 的终态由保留的 Command row 表达。下方旧 `command_terminal` 五态不得用于解释 §6 的目标三终态。
...
## 4. Legacy HTTPS Enrollment（flag day 前实现）

当前已接线的 server-auth HTTPS Enrollment、Device Token 与 Bearer-before-101 只是 ADR-0038 atomic cutover 前的 transitional implementation。其历史 wire、approval/claim、`expired` 与 Phase 3 验收语义保留在已被取代的 [ADR-0033](adr/0033-enrollment-and-device-control-boundary.md) 与 [Phase 3 status](gates/phase-3-status.md)，不再是新的 authority 目标。Flag day 必须同批删除 public Enrollment HTTP、Device Token/Bearer authority 与其 transitional schema；不得让它们与下节 control-key authority 同时生效。

## 5. Device control：WSS

- **传输**：WebSocket over server-auth TLS；Protobuf 消息作为 tungstenite 重组后的 WS binary message（无自定义 length-prefix framing）；唯一 `Sec-WebSocket-Protocol` 为 `natsume.control.v1`，它是 control wire generation 的唯一版本权威，不匹配在 upgrade 拒绝。`ServerChallenge` / `ClientProof`、`CredentialBundle` 与 `SyncState` 会在 semantic decode 后 canonical re-encode 并参与 proof/hash；任一这类 typed message 的字段集合变化（含 additive optional field）都必须使用新 subprotocol，不能依赖旧 peer 丢弃 unknown field 后仍兼容；
...
- Handshake 为定向 envelope：Server `ServerChallenge` | `EnrollmentReviewStatus` | `CredentialBundle` | `EnrollmentActivated` | `SessionReady` | `ServerClose`；Client `ClientProof` | `CredentialAck` | `EnrollmentReady` | `ClientClose`。Active Client 也允许 `ClientClose`。无 `ClientInit`、Hello、`ControlEnvelope` 或 `ProofIntent`。Challenge/Proof 不携带 numeric protocol version；`ClientProof.oneof purpose` 为 `EnrollmentAttempt` 或空 `ResumeSession`；`enrollment_id` / `device_id` 是 canonical UUIDv7。Ed25519 public key、SHA-256 digest 与 challenge nonce 严格校验密码学长度；`session_id` 必须是 Server-generated UUIDv7 的 16-byte RFC 9562 network-order encoding；
...
- `EnrollmentAttempt.evidence_quality` 是 Device 本地 identity 配方对已通过 2-of-3 claim 的 present anchors 固有质量取第二高值，表示 quorum 的最低质量；三个 slot 都 present 时仍取第二高。固定 anchors 为 `strong + strong + medium`，故 `EnrollmentEvidenceQuality` 只有 `MEDIUM` / `STRONG` 两个合法非默认值，`UNSPECIFIED` 与未知 enum 拒绝，不存在 `WEAK`。它必须在首次 attempt 前随其他 immutable transaction material crash-safe 持久化，exact replay 读取该值而不重算。Server 不接收 raw anchors，不能独立验证该 candidate-signed self-report；Panel 必须标为 Device self-reported advisory evidence。它不放宽 identity admission，也不授予 authority；
...
- `revoked` 是旧 Device 身份的永久终态：旧 active key、旧 Resume 与旧 active Enrollment replay 一律不能恢复它。只有现场显式清除旧 identity-bound material、生成全新的 control/Gateway key、CSR 和 `enrollment_id` 后，新的 `EnrollmentAttempt` 才可在 open window 进入人工审核；Server 从同 HWID 的 revoked predecessor 派生新旧 Device 关系并向 Panel 展示历史，不增加 Client intent 字段。Approve/Ack 创建全新的 `device_id`，旧 Device/keys/certificates/Command/audit rows 保留且不改写。物理 schema 因此只允许一个**非 revoked** Device row 占有某 HWID；revoked 历史可与新 current row 共享该 HWID。`disabled` 不释放 HWID，也不能走这条路径；
...
- Enrollment 后半段为 `Bundle → Ack → Activated → Ready → SessionReady`。Client durable state 是 `current_authority: Option<ActiveManifest>` 与 `candidate_enrollment: Option<Prepared | BundleInstalled>` 两槽。candidate connection 始终用 exact Enrollment purpose；replacement 期间旧 current-authority connection 可并存并只用 Resume。Client 必须先完整验证并 crash-safe 持久化 Bundle 才发送 Ack；收到 Activated 后以一个 local transaction 新建/替换 current authority 并清除 exact candidate，之后才发送 Ready。若 Server Ack 已提交而 Client 在该本地事务前崩溃，candidate 仍存在并优先 exact replay，不能因旧 manifest 尚在而只发旧 Resume。任一写入失败都不得越过对应 barrier，可安全编码时发送一次 `ClientClose` 后关闭。`awaiting_credential_ack` exact Ack 激活一次，active exact Ack 是 no-op；Ready/SessionReady 丢失后 candidate 已清除的新 Active Client 以 Resume 获取新 lease；
- 每个 tungstenite 重组后的 WSS Protobuf message 统一以 64 KiB 为 decode-before-allocation 上限。超限、无法安全分类或无法可靠编码响应的失败直接关闭；其余可分类的非法 oneof、phase violation、pre-admission 拒绝或 lifecycle eviction 最多发送一次 `ServerClose`，此后不再发送业务 Protobuf 并关闭 WebSocket。Wire generation mismatch 在 101 前由 exact subprotocol 协商拒绝。`EnrollmentReviewStatus(DENIED)` 是持久化的人工审核结果：发送它后直接关闭，不再追加 `ServerClose`；
- 动态 registry 以 Machine Hardware ID 创建 process-lifetime `MachineActor`；其内部 `DeviceLane(device_id)` 隔离每个 Device lifecycle 的 lease、Observed、Command FIFO 与 eviction。DeviceId/public-key alias 必须解析到准确 lane/transaction，HWID 相同本身不允许跨 lane 读取或修改状态；
- 每个非终态 Enrollment transaction 同时至多一个 current Handshake attachment。新 exact replay 在 MachineActor transaction slot 内原子替换旧 attachment，旧 socket best-effort 收到 `ServerClose{error_code=CONTROL_CONNECTION_SUPERSEDED,action=STOP}`。Operator review 只向 current attachment 发送；无 attachment/send failure 不回滚 durable transaction，下次 exact replay 收敛。Replacement/recovery Handshake attachment 不驱逐 current DeviceLane 的旧 Active lease，只有 exact Ack activation transaction 才 supersede 旧 key/lease；
- 每个 non-revoked DeviceLane 同时至多一个 current control lease，且 lease 内存事实必须带 owner `device_id`。新连接在发送 `SessionReady` 前只在准确 lane 原子安装新 UUIDv7 lease 并使该 lane 旧 lease 失效；旧 socket 可写时 best-effort 收到 `ServerClose{error_code=CONTROL_CONNECTION_SUPERSEDED,action=STOP}`，其 Active outer envelope 精确回显被终止的旧 `session_id`，但 authority 转移不依赖其送达。同一连接 exact `EnrollmentReady` replay 重放同一 lease；任何重连生成新 lease；lease 不持久化，也不进入 Identifier 目录。面向 revoked predecessor 的 repeat lifecycle/noop 仅在 lease owner 精确等于旧 `device_id` 时驱逐，绝不能关闭 successor lane；
- `SessionReady` 后 Client 第一条 Active envelope 必须是 fresh Observed。准确 DeviceLane 在校验并持久化前处于 `AwaitingInitialObserved`，不投递 Command、不处理 BindingRequest。无效首包或持久化失败关闭连接；无 Observed sequence/session field 或额外 ACK；
- Active envelope 只精确回显 `SessionReady.session_id` 做 current-lease 校验，不携带 `authority_revision`。旧 lease 上仍 `queued` 的 Oneshot 确定失败为 `COMMAND_NOT_DELIVERED`，旧 lease 上已 `in_flight` 而终态丢失的 Oneshot 为 `outcome_unknown`；Converge 在新连接完成 initial Observed 后按 frozen convergence key 推定成功、退回 `queued` 重投或 fail closed。无 Heartbeat、`SessionTarget`、`TargetAssignment` 或 `TargetGateway`；
...
- `CommandStatus` 为 `{command_id, state, error_code}`：`CommandState` 只有 `SUCCEEDED` | `FAILED`；`SUCCEEDED` 要求空 `error_code`，`FAILED` 要求非空受限 ErrorCode token；它不是 `stable_error_code` 或人类文本。Server 只接受 current lease 上与 current `in_flight.command_id` 匹配的 status；已终态的 exact same status 零写入 no-op，冲突终态或非 current ID 是 protocol violation；
- `OPEN_BINDING_PROMPT` 空 body，只以 `CommandStatus` 应答；现场确认是 `BindingRequest{seat_code}` → `BindingResult{state,error_code}`。`APPROVED` 要求空 `error_code`，`REJECTED` / `CONFLICT` 要求非空受限 ErrorCode token；unknown well-formed token 仍保持 opaque。每个 Active session 最多一个 in-flight BindingRequest，按 channel 顺序配对，不携 request ID 或 occupancy `binding_id`；
- `EnrollmentReviewStatus(PENDING_REVIEW)` 要求空 `error_code`，`DENIED` 要求非空受限 ErrorCode token；`ServerClose` 的 `error_code` 在所有 action 下都必须非空。任一 state/error_code 非法组合均是 semantic validation failure，不由接收方修补；未知但语法合法的 token 不是 semantic failure；
- `ClientClose.error_code` 同样必须是非空受限 ErrorCode token。Client 只在无法安全发送正常 Ack、Ready、CommandStatus 或 Observed 时 best-effort 发送它；每连接最多一次，随后不再发送业务 Protobuf 并关闭，无 ACK。它没有 action：Server 不从该包推导 Client 重试、Enrollment 审核、Device lifecycle 或 Command terminal result。Server 可将该 bounded token 记录为 structured connection diagnostic；proof 后可关联已经认证的 attachment/current lease，proof 前则不可归因于 Device。无论哪种阶段都只按普通断线 barrier 收敛，不创建 durable 业务事实；
- `ServerClose.action` 必须是非 `UNSPECIFIED` 的 `ServerCloseAction` enum。`RETRY` 进入 Client 唯一的指数退避 + jitter 策略；`STOP` 终止同一 local connection intent，直到软件或 durable local state 的可观察变化，或 operator 明确重新 arm。协议不携带 Server retry delay/deadline，Client 不按 `error_code` 推导 action。陈旧 local socket 收到的 Close 只终止该 socket incarnation，不得取消更新连接或删除 durable Enrollment material。Server 每连接最多发送一个 `ServerClose`，无需 ACK；keep-alive 使用 WS ping/pong；重连后只有非终态 Converge 在领域键仍 drift 时重投，Oneshot 不重放；
...
Command current row 直接绑定 `command_id`、`device_id`、Device 内严格递增的 `enqueue_order`、`kind`、`state`、request fingerprint version/hash、可选 `group_correlation_id`、`payload_version`、`frozen_payload_json`、`created_at_unix_ms`、可选 terminal error/result 和 `created_audit_event_id`。它不保存 deadline、秘密 payload copy、certificate 或 token issuance 数据；每 Command 的 frozen typed input 只在 `frozen_payload_json` 内表示。`enqueue_order` 是 `1..=i64::MAX` 的内部持久化顺序，不进 Protobuf、HTTP 或公开 Identifier；每个 Device 上唯一，允许 crash gap，不要求连续。当前 migration 尚缺该列/unique constraint，且残留的 `deadline_at_unix_ms` 无写者和状态转换语义，二者属待 flag day 同步的实现漂移。

所有首次创建请求（包括目标 Device 离线时）都路由到 HWID 的 MachineActor 有界 single-consumer mailbox，再进入准确 `DeviceLane(device_id)`。Lane 在首次装载时只读取自身 `device_id` 已持久化最大 `enqueue_order`，在同一串行 turn 中分配下一值，并把 Command 与创建 audit 原子提交；进程内同一 `device_id` 不得存在两个 lane，数据库 `UNIQUE(device_id,enqueue_order)` 是最终竞态守卫。HTTP exact replay 返回原 row/order，不消费新序号；same-ID fingerprint conflict 同样不消费。Lane 重建后先按 `enqueue_order` 恢复：若有自己的 `in_flight`，必须先完成其断线/Observed 收敛，再考虑更晚的 `queued`；否则投递最早 `queued`。revoked predecessor 与 successor 的 allocator、in-flight、queued 状态互不共享。达到 SQLite INTEGER 上界时首次创建 fail closed，不回绕或改用时间戳/Command ID 猜顺序。

Target `commands.state` 只有 nonterminal `queued` / `in_flight` 与 terminal `succeeded` / `failed` / `outcome_unknown`。`in_flight` 是 Server 在网络发送前 durable 提交的保守 delivery-attempt cut：它表示副作用已可能发生，不声称 Device 已收到。每台 Device 同时至多一条 `in_flight`；`queued → in_flight` commit 失败必须零 wire 副作用。普通连接丢失时，`outcome_unknown` 用于 Oneshot 已过 cut 且终态丢失的情况；terminal lifecycle 又会用它收口任何已经 `in_flight`、但因旧 Device 永久失去 authority 而再也无法观测收敛的 Command。它不是 Client wire `CommandState`，不得自动重放。当前 migration 的八态 CHECK 属待 flag-day 收紧的实现漂移。

disable/revoke lifecycle transaction 在提交 state/key/certificate 变化与 audit 的同一 guarded transaction 内、evict 之前收口该旧 `device_id` lane 的 Command：全部 `queued`（Converge 与 Oneshot）转为 `failed/COMMAND_NOT_DELIVERED`，全部 `in_flight` 转为 `outcome_unknown`，既有终态不变。若 terminal Command 先提交，lifecycle 观察并保留它；若 lifecycle 先提交，旧 lease 随后送达的 status 已无 current authority 而被拒绝。事务提交后才以目标 `device_id` compare-and-evict：只有 lease owner 相同才关闭；旧 revoked Device 的 repeat noop 不得影响同 HWID successor lane。该收口不删除 Command，因而 disable/revoke 后既有 `command_id` 的 HTTP exact replay/conflict 仍确定；未来若增加 enable，恢复 Device 也不得复活这些旧意图，operator 必须创建新 Command。

V2 业务 family 限于：`SYNC_STATE`、`SYNC_SECRET`、`OPEN_BINDING_PROMPT`、`SESSION_LOCK`/`UNLOCK`/`TERMINATE`、`HOME_RESET`（具体枚举以 `.proto` 为准；新增 family 必须证明不是任意远程管理能力）。wire/DB `kind` 值即 proto oneof 字段名（lower snake，例如 `SESSION_LOCK`↔`lock_session`、`HOME_RESET`↔`reset_home`），文档族名为其 UPPER_SNAKE 标签。`OpenBindingPrompt` / `LockSession` / `UnlockSession` / `TerminateSession` 均为空 body。`OPEN_BINDING_PROMPT` 只驱动封闭的 binding-prompt screen，因此不构成任意远程管理能力。无 TTL、无 `prompt_message_id`、无 `seat_code`。Device 打开该 screen 即报 `CommandStatus` `SUCCEEDED`；志愿者确认后 Device 发起 `BindingRequest`，Server 回 `BindingResult`，Seat 在 `BindingRequest.seat_code`；本地取消不发业务包。`CommandState` 只有 `SUCCEEDED` | `FAILED`。终态由可选 `terminal_error_code` 或 `redacted_terminal_result_json` 表示（[ADR-0034](adr/0034-state-execution-and-data-plane-boundary.md)）。

Binding 提交不使用 durable request transaction。Server 在同一事务中检查 Device/Seat 唯一性并处理：未绑定 Device + 空闲 Seat 创建新 Binding 并 `APPROVED`；已是同一 Device↔Seat 关系则零写入 `APPROVED`；任一端已被其他关系占用则 `CONFLICT`；Seat 不存在或 policy 拒绝则 `REJECTED`。因此响应丢失后重投相同 Seat 是 no-op approved，改投其他 Seat 不会偷偷改绑。`OPEN_BINDING_PROMPT` 是 UI 动作，不是 Server-side capability token；不建 prompt transaction。Binding 成功只改变 Server truth，不隐式创建 `SYNC_STATE` / `SYNC_SECRET`。

Server MachineActor 用单消费者有界 inbox 排序事件，并在准确 DeviceLane 内处理每台 Device 的 operator action、socket event 与 lifecycle event。Client WS reader 把 Command 放入单消费者有界 channel。每个 lane 同时最多一条 `in_flight` Command，Client 在完成副作用与 terminal status 后才执行下一条。不增加 wire sequence、generation 或 Device-wide clock。

Oneshot 的 live-lease 语义在 actor 中冻结：创建时无通过 initial Observed barrier 的 current lease，或它仍为 `queued` 时所属 lease 断开/被替换/Server 重启，均终止为 `failed` 并使用 `COMMAND_NOT_DELIVERED`；不转投新 lease。这些 Command 仍持久化 operator 意图与审计，因此首次 PUT 仍为 `201`，响应可以已是 terminal `failed`。

**投递二分（不是一套 Device journal 耐久机）**：

- **Converge**（按领域键幂等；只有同一非终态 operator 意图可依 drift 重投同一 `command_id` / payload；Device **不**维护 command journal）：
...
- **Oneshot**（仅 current live socket；无 lease 或 pre-send 丢失 lease 为 `COMMAND_NOT_DELIVERED`；`in_flight` 后不确定为 `outcome_unknown`；重连不重放）：`lock_session`、`unlock_session`、`terminate_session`、`open_binding_prompt`。本地 executor 在开始时捕获 current graphical session，特权动作前重检；replacement 在执行中发生则 `SESSION_CONTEXT_STALE`，不改投新 session。

Converge 的 `in_flight` 在断线后不直接终态化：新 current lease 通过 initial Observed barrier 后，若 fresh Observed 已匹配该 Command 的 frozen convergence key，Server 推定 `succeeded`；仍为普通 drift 则 durable 退回 `queued` 并重投同一 ID/payload；Observed 超前、回退或无法安全比较时 fail closed。已收到明确 `SUCCEEDED` / `FAILED` 的 terminal Command 不得因后来 drift 复活；重新执行需 operator 创建新 Command。

Server 只接受 current lease 上与 current `in_flight.command_id` 匹配的 `CommandStatus`。已终态的 exact same status 零写入 no-op；同 ID 不同终态或非 current in-flight ID 是 protocol violation。每次 delivery attempt 最多一个 terminal status；Converge 可在断线恢复后以同一 `command_id` 发起新 attempt。

`PUT /api/v2/commands/{command_id}` 仍作 operator 审计；HTTP 同 ID fingerprint 语义不变。Server 从已存储的 frozen payload 确定性渲染 Converge 重投。Device 不得以 command journal 作为去重权威；仓库内既有 journal 实现属预发布待拆除。同 `command_id` 不同 fingerprint 仍由 Server `COMMAND_REQUEST_CONFLICT` 处理。

## 7. `SYNC_STATE`

`SYNC_STATE` 是 Converge 命令：wire 为 `SyncState{oneof assignment}`，`unbound{}` 与 `bound{binding_id,account_id,seat_code,domjudge_username}` 互斥；无 `generation`、密码、证书或 packaged profile id。`canonical_hash` 不在 wire；双方先构造并语义校验只含该 assignment 的 typed `SyncState`，再计算 `SHA-256("NATSUME-SYNC-STATE-v1\0" || sync_state.encode_length_delimited_to_vec())`。这使被编码对象与 pinned Prost API 唯一明确。已应用同 hash 是 no-op；Observed `applied_hash` 为空表示从未成功应用 SyncState，否则必须恰为 32 bytes。Account mapping 变化时旧 credential context 立即不适用；Device 应用新 assignment 后保持 BLOCKED，直到匹配新 Account 的 secret 安装。

**`SYNC_STATE` 不签发、不携带、不安装任何证书或 token**；Gateway certificate 只在 Enrollment 获得（`INV-CERT-01`）。

## 8. `SYNC_SECRET`

`SYNC_SECRET` 是 Converge 命令：wire 为 `SyncSecret{binding_id,account_id,credential_revision,SecretBytes password}`。`credential_revision` 是 SQLite INTEGER-backed durable value，wire 虽为 `uint64`，语义域只允许 `1..=i64::MAX`；零或超过该上界的值必须在产生副作用前拒绝。`password` message 必须存在；`SecretBytes.value` 必须是 Account vault 中 exact password 的 UTF-8 bytes，长度 `1..=512`，且不得含 Unicode control character。这直接继承 Import 已冻结的 Account password 值对象，不建立另一套可配置限制；missing wrapper、empty bytes、invalid UTF-8、control character 或超限均须在任何副作用前以 malformed Command 拒绝。Device 在写入前重新验证当前 applied assignment 的 Binding 与 Account 均匹配，并校验 revision。只有 Observed credential 的三元组全部相同且 `state=INSTALLED` 才是 no-op。`SecretBytes` 的值不得进入 Debug、日志、Observed、audit、`ServerClose`、`ClientClose` 或 terminal detail。凭据文件与完整 credential context 必须原子、crash-safe 持久化后才可发送 `CommandStatus(SUCCEEDED)` 或报告 `INSTALLED`；失败保留旧 secret 或明确 BLOCKED，不留半写。若本地状态损坏到无法安全构造 FAILED status/Observed，Client 阻断数据面并以 `ClientClose` 终止连接，而不是伪造 `ABSENT`。
...
`completed_home_epoch` 缺失表示从未成功完成 reset；出现时必须在 `1..=i64::MAX`，且只有完整 Prepare/Activate/Recover/GC 与本地 completion record 全部 crash-safe 持久化后才能单调前进并发送 `CommandStatus(SUCCEEDED)`。较新 reset 执行或恢复期间继续报告上一个已完成 epoch。**不得**回传 `secret_state`、`session_instance_id`、`active_lock_command_id`、`boot_id`、`observed_sequence`、`received_generation`、`apply_status`、session_agent blob、通用 `home_state`、Home 进度 blob 或 generation。密码值、token、private key、完整路径和内部异常链不得出现。上报节奏为变化时 + 低频周期兜底；Device 自报不构成授权。
...
**Device Daemon ↔ Session Agent**：UI snapshot 只含展示所需数据，不含 password、token、certificate private material、Server 凭据或任意 HTML。调用校验 UID/PID/logind session 与当前 graphical session；陈旧 Agent 或 lease 被拒绝。WSS Oneshot 开始时捕获 current session，特权动作前重检，不携 wire target。`OPEN_BINDING_PROMPT` 打开 screen 即终态成功；现场提交经 `BindingRequest{seat_code}`，不是该 Command 的成功。**lock/unlock 不调用 Caddy adapter**。
...
发送方规则：字符串值显式定义；每个公开 adapter 对本地已知错误映射穷举；未分类内部错误映射到有限通用码；`detail` 默认无或脱敏；新内部错误不自动成为新稳定码；首次对外发布后删除稳定码需要兼容计划；**Web/Device 不解析 Display 文本**；同一语义跨 transport 使用同一稳定码。实现级 variant 不直接进入 registry，只有调用方可稳定处理、会跨公开边界或持久化到 `CommandStatus`/Observed 的语义才进入下表。

Protobuf ErrorCode 的 wire grammar 是非空、不超过 64 ASCII bytes 且匹配 `[A-Z][A-Z0-9_]*`。接收方对本地未知但语法合法的 token 必须作 opaque 值接受：保留包内其他 typed fields（如 state/action）；只有所属业务包本就定义 durable terminal result 时才可随该结果持久化。UI 只显示通用错误，不直接显示原 token，不把它作为 metrics label，不驱动 retry、authorization 或 domain decision。未知 token 本身不得导致含它的合法 envelope 失败；`ClientClose` 仍不创建 durable business fact。
...
| enrollment | `PROVISIONING_WINDOW_CLOSED` | provisioning window 非 open 时拒绝新的 Enrollment admission，或拒绝 pending transaction 的 approve/sign；不得签发。已固化 Bundle 的 exact replay、Ack 与最终化不受此码影响。 |
| enrollment | `ENROLLMENT_REQUEST_INVALID` | Enrollment 的有界 typed request、CSR/SPKI 或协议输入无效；不得留下部分 issuance。 |
| enrollment | `ENROLLMENT_REQUEST_REJECTED` | operator 显式拒绝了该 Enrollment transaction；目标 wire 以 `EnrollmentReviewStatus{DENIED,error_code}` 返回，Device 停止该 attempt 并等待现场人员介入，零签发。 |
...
| control | `PROTOCOL_VERSION_UNSUPPORTED` | exact WSS subprotocol 不受支持；101 前拒绝协商，不猜测兼容。Challenge/Proof 无 numeric protocol version，101 后不存在第二套 version negotiation。 |
...
| control | `CONTROL_CONNECTION_SUPERSEDED` | 新 exact Enrollment replay 已原子替换旧 Handshake attachment，或同一 Device 的新认证 Active 连接已替换旧 current control lease；旧连接后续 frame 无 authority，best-effort 收到 `ServerClose{action=STOP}` 后关闭。该码不驱动 Client 行为。 |
| control | `COMMAND_NOT_DELIVERED` | 普通投递中，Oneshot 在创建时无通过 initial Observed barrier 的 current lease，或仍为 `queued` 时所属 lease 断开/被替换/Server 重启；terminal Device lifecycle 还将该旧 Device 的全部 queued Command 以此码收口。两种情况都可确定未进入 durable delivery-attempt cut，终止为 `failed` 且不重放。 |
...
WSS reassembled Protobuf message 的唯一 frame 上限是 64 KiB；oversized message 是 transport ingress resource-limit failure，直接关闭且不进入 ErrorCode registry。ErrorCode 还必须单独满足上述 64-byte ASCII token grammar，不得以 frame 上限允许大字符串或自由文本。`ServerClose.error_code` 与 `ClientClose.error_code` 都复用本表 token：前者的 Client 行为只看 required enum `action`，后者根本不携行为指令；两者都不维护 error-code-to-action 常量表。其余 catalog 目标由 `natsume-error-code` crate 实现；当前 Rust 尚未跟随本轮协议修订，待 flag-day implementation batch 原子同步。每个本地已知 variant 使用显式 Serde rename，不从 Rust 名称或 Display 推导 wire value；未知合法 token 以 opaque remote value 表示。

## 13. 版本和兼容

当前 active-development Device-control Proto 尚未对外发布：删除字段不保留 `reserved`，field number 应随当前结构压紧整理；这类修改必须作为单一 breaking change 同步 descriptor golden 与全部 peer，不承诺旧构建互通。Exact `natsume.control.v1` 是当前 control wire generation 的唯一版本权威；Challenge/Proof 无 numeric version，proof transcript 以无独立版本的用途 domain 加实际 subprotocol 做 domain separation。`ServerChallenge` / `ClientProof`、`CredentialBundle`、`SyncState` 等 canonical-encoded hash/proof root 及其递归可达 nested message 的**字段集合变化一律视为 breaking**，即便 Protobuf 通常把新增 optional field 视为 additive；旧 runtime decode 时丢弃 unknown field 后无法重建相同 bytes，因此必须切换 WS subprotocol，而不是再添加 message-local numeric version。

从首个对外发布基线开始，已发布的 field number、interface name、method/signal/error name、ID 和 revision 语义才进入不可复用承诺；之后的破坏性 wire 变化使用新 WS subprotocol 或 interface version。downgrade/rollback 通过发布 runbook 定义，不假设 schema 自动回滚。

## 14. 契约验证

CI 必须证明：生成契约 clean diff；Command PUT replay/conflict；MachineActor/DeviceLane alias 路由、lane-local 首次创建分配/重启恢复/上界拒绝与 `UNIQUE(device_id,enqueue_order)`；predecessor/successor 的 lease/Observed/Command/order 隔离及旧 Device repeat-noop compare-and-evict；`queued → in_flight` pre-send durable cut、每 Device 单 in-flight 与 status-current-lease/ID 配对；Converge initial-Observed 推定成功/退队重投/fail-closed；Oneshot pre-send `COMMAND_NOT_DELIVERED`、post-cut `outcome_unknown` 与不重放；disable/revoke 对准确 lane queued/in-flight 的原子收口及 late status 拒绝；typed assignment hash golden；完整 credential context 收敛、required/non-empty UTF-8/control-free/512-byte password validation、write-before-success 与 SecretBytes redaction；Binding 同关系 no-op replay 与冲突；manual Enrollment review/approve+issue atomicity/denial/closed-window replay/no expiry/Ack crash cuts、Client current-authority/candidate 双槽、replacement Ack 后旧 manifest + candidate 崩溃恢复、Bundle/manifest write-before-Ack/Ready、单 current Handshake attachment/新 replay 替换/与旧 Active lease 并存；revoked predecessor 的新 Device reprovision、旧 Resume 永久拒绝与同 HWID 单 non-revoked ownership；canonical SemVer、`EnrollmentEvidenceQuality` 仅 MEDIUM/STRONG 与 second-highest 配方；canonical proof/bundle/state message field-set 变化必须切换 subprotocol；UUIDv7 lease 长度/version/variant/新连接替换/旧帧拒绝；initial Observed barrier；64 KiB frame limit、ErrorCode 64-byte ASCII grammar/未知 opaque handling、unknown enum、Handshake/Active `ServerCloseAction` 的非 `UNSPECIFIED`/单次发送/close-after-send、Handshake/Active `ClientClose` 的单次发送/no-action/no-state-transition，以及 Denied 不追加 Close；Command/Binding/Review state-error 矩阵；revision/epoch 的 zero/`i64::MAX`/overflow；Gateway state/hash 独立组合与完整 leaf DER SHA-256；`completed_home_epoch` 的 absence/monotonic/in-progress/recovery/write-before-success/Server-ahead fail-closed；Session replacement 重检；D-Bus policy、ErrorCode 本地映射穷举、secret/path redaction、Caddy 安全与禁止通用执行能力。
