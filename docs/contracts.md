...
**2026-08-25 修订（Client terminal report、durable queue 与 terminal lifecycle）**：Handshake/Active Client envelope 统一允许 `ClientClose{error_code}`；它只报告本连接无法安全继续，不带 retry action，不替代 Ack、CommandStatus 或任何 durable transaction transition。`commands.enqueue_order` 冻结为 Device 内持久化顺序，所有首次创建经 DeviceActor 串行分配。disable/revoke 事务在驱逐前把该旧 Device 的非终态 Command 收口；revoked 身份永不恢复，显式本地 reprovision 只能经新的人工审核 Enrollment 创建新 `device_id`。

**已接受目标**由 [ADR-0038](adr/0038-unified-ordinary-wss-device-control-authority.md) 定义：普通 WSS 内的 Challenge/Proof、统一人工审核且不过期的 durable Enrollment transaction、`EnrollmentReviewStatus`、`CredentialBundle`/`CredentialAck`、`EnrollmentActivated`/`EnrollmentReady` barrier、`SessionReady{session_id = 16-byte UUIDv7}`、双向 terminal close、initial Observed barrier、Ed25519 control key 与动态 DeviceActor。`ClientProof` 使用结构化 `oneof purpose`：`EnrollmentAttempt` 必带 durable `enrollment_id`、candidate public key、exact CSR 与首次 attempt 前一并持久化的 aggregate `evidence_quality`；`ResumeSession` 为空且只供已持久化 Active manifest 使用。对应 Proto foundation 已存在，但尚无 runtime authority consumer。
...
- **传输**：WebSocket over server-auth TLS；Protobuf 消息作为 tungstenite 重组后的 WS binary message（无自定义 length-prefix framing）；唯一 `Sec-WebSocket-Protocol` 为 `natsume.control.v1`，它是 control wire generation 的唯一版本权威，不匹配在 upgrade 拒绝；
...
- Enrollment 后半段为 `Bundle → Ack → Activated → Ready → SessionReady`。Client 必须先完整验证并 crash-safe 持久化 Bundle 才发送 Ack，再原子持久化 Active manifest 才发送 Ready；任一写入失败都不得越过对应 barrier，可安全编码时发送一次 `ClientClose` 后关闭。`awaiting_credential_ack` exact Ack 激活一次，active exact Ack 是 no-op；Ready/SessionReady 丢失后 Active Client 以 Resume 获取新 lease；
...
- 每个非终态 Enrollment transaction 同时至多一个 current Handshake attachment。新 exact replay 在 actor 内原子替换旧 attachment，旧 socket best-effort 收到 `ServerClose{error_code=CONTROL_CONNECTION_SUPERSEDED,action=STOP}`。Operator review 只向 current attachment 发送；无 attachment/send failure 不回滚 durable transaction，下次 exact replay 收敛。Replacement/recovery Handshake attachment 不驱逐旧 Active lease，只有 exact Ack activation transaction 才 supersede 旧 key/lease；
- 每个 DeviceActor 同时至多一个 current control lease。新连接在发送 `SessionReady` 前先原子安装新 UUIDv7 lease 并使旧 lease 失效；旧 socket 可写时 best-effort 收到 `ServerClose{error_code=CONTROL_CONNECTION_SUPERSEDED,action=STOP}`，其 Active outer envelope 精确回显被终止的旧 `session_id`，但 authority 转移不依赖其送达。同一连接 exact `EnrollmentReady` replay 重放同一 lease；任何重连生成新 lease；lease 不持久化，也不进入 Identifier 目录；
...
所有首次创建请求（包括目标 Device 离线时）都路由到该 DeviceActor 的有界 single-consumer mailbox。Actor 在首次装载时读取已持久化最大 `enqueue_order`，在同一串行 turn 中分配下一值，并把 Command 与创建 audit 原子提交；进程内同一 `device_id` 不得存在两个 actor，数据库 `UNIQUE(device_id,enqueue_order)` 是最终竞态守卫。HTTP exact replay 返回原 row/order，不消费新序号；same-ID fingerprint conflict 同样不消费。Actor 重建后先按 `enqueue_order` 恢复：若有 `in_flight`，必须先完成其断线/Observed 收敛，再考虑更晚的 `queued`；否则投递最早 `queued`。达到 SQLite INTEGER 上界时首次创建 fail closed，不回绕或改用时间戳/Command ID 猜顺序。
...
Server DeviceActor 用单消费者有界 inbox 排序每台 Device 的 operator action、socket event 与 lifecycle event。Client WS reader 把 Command 放入单消费者有界 channel。Server 同时最多一条 `in_flight` Command，Client 在完成副作用与 terminal status 后才执行下一条。不增加 wire sequence、generation 或 Device-wide clock。
...
CI 必须证明：生成契约 clean diff；Command PUT replay/conflict；DeviceActor 首次创建分配/重启恢复/上界拒绝与 `UNIQUE(device_id,enqueue_order)`；`queued → in_flight` pre-send durable cut、每 Device 单 in-flight 与 status-current-lease/ID 配对；Converge initial-Observed 推定成功/退队重投/fail-closed；Oneshot pre-send `COMMAND_NOT_DELIVERED`、post-cut `outcome_unknown` 与不重放；disable/revoke 对 queued/in-flight 的原子收口及 late status 拒绝；typed assignment hash golden；完整 credential context 收敛、write-before-success 与 SecretBytes redaction；Binding 同关系 no-op replay 与冲突；manual Enrollment review/approve+issue atomicity/denial/closed-window replay/no expiry/Ack crash cuts、Bundle/manifest write-before-Ack/Ready、单 current Handshake attachment/新 replay 替换/与旧 Active lease 并存；revoked predecessor 的新 Device reprovision、旧 Resume 永久拒绝与同 HWID 单 non-revoked ownership；canonical SemVer 与 evidence-quality second-highest 配方；UUIDv7 lease 长度/version/variant/新连接替换/旧帧拒绝；initial Observed barrier；64 KiB frame limit、ErrorCode 64-byte ASCII grammar/未知 opaque handling、unknown enum、Handshake/Active `ServerCloseAction` 的非 `UNSPECIFIED`/单次发送/close-after-send、Handshake/Active `ClientClose` 的单次发送/no-action/no-state-transition，以及 Denied 不追加 Close；Command/Binding/Review state-error 矩阵；revision/epoch 的 zero/`i64::MAX`/overflow；Gateway state/hash 独立组合与完整 leaf DER SHA-256；`completed_home_epoch` 的 absence/monotonic/in-progress/recovery/write-before-success/Server-ahead fail-closed；Session replacement 重检；D-Bus policy、ErrorCode 本地映射穷举、secret/path redaction、Caddy 安全与禁止通用执行能力。
