...
> **ADR-0038 flag-day 边界**：本小节是“当前生产写入器”注册表，不是目标 Enrollment/Command 状态机。Atomic cutover 时必须同批删除 `expire_enrollment_requests`、`same_spki_retry`、Enrollment `window_closed` expiry 等旧词汇，并先注册 manual admission/review/approve+issue/deny/Ack-activation 与 Oneshot `outcome_unknown` 的 typed audit 形状。下方旧 `command_terminal` 五态不得用于解释 §6 的目标三终态。
...
Command current row 直接绑定 `command_id`、`device_id`、`kind`、`state`、request fingerprint version/hash、可选 `group_correlation_id`、`payload_version`、`frozen_payload_json`、`created_at_unix_ms`、可选 terminal error/result 和 `created_audit_event_id`。它不保存 deadline、秘密 payload copy、certificate 或 token issuance 数据；每 Command 的 frozen typed input 只在 `frozen_payload_json` 内表示。当前 migration 残留的 `deadline_at_unix_ms` 无写者和状态转换语义，属待 flag day 删除的实现漂移。

Target `commands.state` 只有 nonterminal `queued` / `in_flight` 与 terminal `succeeded` / `failed` / `outcome_unknown`。`in_flight` 是 Server 在网络发送前 durable 提交的保守 delivery-attempt cut：它表示副作用已可能发生，不声称 Device 已收到。每台 Device 同时至多一条 `in_flight`；`queued → in_flight` commit 失败必须零 wire 副作用。`outcome_unknown` 只由 Server 在 Oneshot 已进入 `in_flight`、但终态 status 前发生 send failure、disconnect 或 Server restart 时产生；它不是 Client wire `CommandState`，不得自动重放。当前 migration 的八态 CHECK 属待 flag-day 收紧的实现漂移。
...
Protobuf ErrorCode 的 wire grammar 是非空、不超过 64 ASCII bytes 且匹配 `[A-Z][A-Z0-9_]*`。接收方对本地未知但语法合法的 token 必须作 opaque 值接受：保留包内 state/action，可作为受限 terminal token 持久化，但 UI 只显示通用错误，不直接显示原 token，不把它作为 metrics label，不驱动 retry、authorization 或 domain decision。未知 token 本身不得导致含它的合法 envelope 失败。
...
| control | `COMMAND_NOT_DELIVERED` | Oneshot 在创建时无通过 initial Observed barrier 的 current lease，或仍为 `queued` 时所属 lease 断开/被替换/Server 重启；Server 可确定它未进入 durable delivery-attempt cut，终止为 `failed` 且不重放。 |
...
WSS reassembled Protobuf message 的唯一 frame 上限是 64 KiB；oversized message 是 transport ingress resource-limit failure，直接关闭且不进入 ErrorCode registry。ErrorCode 还必须单独满足上述 64-byte ASCII token grammar，不得以 frame 上限允许大字符串或自由文本。`ServerClose.error_code` 复用本表 token，但 Client 行为只看其 required enum `action`，不维护 error-code-to-action 常量表。其余 catalog 目标由 `natsume-error-code` crate 实现；当前 Rust 尚未跟随本轮协议修订，待 flag-day implementation batch 原子同步。每个本地已知 variant 使用显式 Serde rename，不从 Rust 名称或 Display 推导 wire value；未知合法 token 以 opaque remote value 表示。
...
CI 必须证明：生成契约 clean diff；Command PUT replay/conflict；`queued → in_flight` pre-send durable cut、每 Device 单 in-flight 与 status-current-lease/ID 配对；Converge initial-Observed 推定成功/退队重投/fail-closed；Oneshot pre-send `COMMAND_NOT_DELIVERED`、post-cut `outcome_unknown` 与不重放；typed assignment hash golden；完整 credential context 收敛与 SecretBytes redaction；Binding 同关系 no-op replay 与冲突；manual Enrollment review/approve+issue atomicity/denial/closed-window replay/no expiry/Ack crash cuts、单 current Handshake attachment/新 replay 替换/与旧 Active lease 并存；canonical SemVer 与 evidence-quality second-highest 配方；UUIDv7 lease 长度/version/variant/新连接替换/旧帧拒绝；initial Observed barrier；64 KiB frame limit、ErrorCode 64-byte ASCII grammar/未知 opaque handling、unknown enum、Handshake/Active `ServerCloseAction` 的非 `UNSPECIFIED`/单次发送/close-after-send，以及 Denied 不追加 Close；Command/Binding/Review state-error 矩阵；revision/epoch 的 zero/`i64::MAX`/overflow；Gateway state/hash 独立组合与完整 leaf DER SHA-256；`completed_home_epoch` 的 absence/monotonic/in-progress/recovery/Server-ahead fail-closed；Session replacement 重检；D-Bus policy、ErrorCode 本地映射穷举、secret/path redaction、Caddy 安全与禁止通用执行能力。
