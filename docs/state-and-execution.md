...
- wire 使用 `SyncSecret{binding_id,account_id,credential_revision,SecretBytes password}`；Device 写入前重新校验当前 assignment 的 Binding 与 Account 都匹配，并校验 credential revision；
