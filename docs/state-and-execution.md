...
- **Command**：单 Device 显式意图，分 Converge（领域键幂等，非终态时可重投同一 payload）与 Oneshot（仅 live socket）。Server MachineActor 在准确 `DeviceLane(device_id)` 内与 Client executor 以单消费者 channel 串行 Command；`commands` current row 用 Device 内 `enqueue_order` 冻结 FIFO、用 `queued → in_flight` 表达 durable delivery cut 与 operator 审计，不声明 Device journal 或独立 delivery history。
...
- **串行、durable order 与 cut**：Server 每个 HWID 一个 MachineActor 有界 mailbox，Command 在准确 DeviceLane 内串行；Client command executor 也有单消费者有界 channel。所有首次创建（离线 Device 也一样）经目标 lane 分配该 Device 内唯一、严格递增的 `enqueue_order` 并与 Command/audit 原子提交；该值限 `1..=i64::MAX`、不进 wire、不要求连续，exact replay/conflict 不消费。lane 重建只读取自身 device_id，先收敛唯一 `in_flight`，否则按 `enqueue_order` 取最早 queued；predecessor/successor 不共享 allocator，到上界 fail closed。Server 同时最多一条 `in_flight` Command，并必须在任何 socket write 前 durable 提交 `queued → in_flight`；转换失败时零 wire 副作用。Client 完成本地副作用并返回 terminal status 后才开始下一条。WS I/O、ping/pong、Observed 与 disconnect 检测可并发。
...
- wire 使用 `SyncSecret{binding_id,account_id,credential_revision,SecretBytes password}`；password wrapper 必须 present，其 value 必须是 Account vault 的 exact、非空、UTF-8、无 control character、最多 512-byte password；revision 必须在 `1..=i64::MAX`，Device 在任何副作用前验证 password 值域，并重新校验当前 assignment 的 Binding 与 Account 都匹配及 credential revision；
...
必须覆盖的安全 fault class：Panel canonical UUIDv7 正/反例、`PUT` 首次 `201` / replay `200` / invalid `400` / conflict `409`、same-ID fingerprint、MachineActor/DeviceLane alias 路由与 revoked predecessor/successor 隔离、lane-local `enqueue_order` 分配/唯一性/重启恢复/上界、旧 lane noop 不驱逐 successor、`queued → in_flight` pre-send durable cut、Server/Client channel 顺序与每 Device 单 in-flight、status-current-lease/ID 配对、Converge initial-Observed 推定成功/退队重投/fail-closed、Oneshot `COMMAND_NOT_DELIVERED` / `outcome_unknown` / 不重放、disable/revoke terminalization 与 late status、assignment hash golden、陈旧 `(binding_id,account_id,credential_revision)`、Account mapping 替换不混用旧密码、required/non-empty UTF-8/control-free/512-byte password、revision/epoch zero/上界/overflow、secret/Home write-before-success、`completed_home_epoch` absent/monotonic/in-progress/recovery/ahead/regression 与 stale、initial Observed barrier、Gateway state/hash 独立组合与 Caddy leaf DER SHA-256/validate/reload 中断、secret redaction/写入中断、session replacement 重检与 Home recovery。具体测试随对应 Phase 实现补全。
