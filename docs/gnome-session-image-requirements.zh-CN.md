...
2. 安装 Client，确认包提供的 PAM、unit 和程序存在；随后从包内 `/usr/share/natsume/image-integration/` 接入 GDM/PAM/登录入口配置。
...
- 输入使用完整 `natsume-client` Deb，核对版本、架构、SHA-256、公共站点配置和信任锚，按包依赖安装运行库；只复制二进制不算包部署。
...
- 缓存键覆盖 Deb **实际内容**、生效公共端点和依赖层/配置；同名 Deb 内容变化须失效。自定义配置目录的宿主输入、缓存键、chroot 只读挂载必须一致。禁用 Natsume 的安装源不依赖此 Deb。
