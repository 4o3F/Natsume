...
> **2026-08-19 ADR-0038 note；2026-08-24/25 transaction 修订**：既有 Phase 4 evidence 证明的是 Device Token、Bearer-before-101 runtime。当前 wire 为定向 handshake/Active envelopes（Server Challenge|EnrollmentReviewStatus|Bundle|Activated|SessionReady|ServerClose；Client Proof|Ack|Ready|ClientClose；Active 双向各允许本方 Close），Proof 使用 EnrollmentAttempt/ResumeSession oneof purpose，并以 durable `enrollment_id` 跨连接重放。所有新 Enrollment transaction 人工审核，状态为 `pending_review` / `awaiting_credential_ack` / `active` / `denied`，无 expiry。旧 descriptor hash 不能冒充当前 evidence，必须重新验证。无 `ClientInit`、无 `ControlEnvelope`、无 Hello。Private ordinary-WSS listener仍只证明 TLS 1.3 + 101 + Challenge/Ed25519 Proof seam，不补交WP5/WP6 evidence，不关闭G4，也不改写下方历史完成度。
>
> **2026-08-24 修订（Command 串行与 Active admission）**：下方 WP4/WP5 历史记录描述的 Device journal、重连重放全部 7 kind、宽 Observed 不再是目标契约。目标：Server 以 Device 内 durable `enqueue_order` 恢复投递 FIFO，Server/Client 各用有界 single-consumer channel 串行执行，每连接最多一个 in-flight；Converge 按完整领域键重推；Oneshot 仅 live socket，结果丢失时 Server 记 `outcome_unknown` 且不重放；无 Device command journal；Observed slim，且 SessionReady 后第一条 Active packet 必须是 Observed；keep-alive 为 WS ping/pong。历史完成度不改写。
>
...
> **2026-08-25 修订（ClientClose、Command order/lifecycle）**：目标 Proto 的 Client Handshake/Active envelope 新增无 action 的 `ClientClose{error_code}`，仅作 terminal connection report。目标 DB 契约新增 Device 内 `enqueue_order`，disable/revoke 在 eviction 前收口 queued/in-flight Command；revoked reprovision 创建新 Device 而不复活旧 identity。下方 `(created_at_unix_ms,command_id)` 投递顺序与 lifecycle evidence 仍是 Token-era 历史，不构成新目标证据；Rust、migration 和现有测试尚未同步。

> **2026-08-25 修订（Machine/Device lane 与 Client 双槽）**：目标 runtime owner 是 HWID-sharded MachineActor 与 per-`device_id` DeviceLane；lease、Observed、Command FIFO/in-flight、`enqueue_order` 与 eviction 全部 lane-local，旧 revoked lane noop 不得驱逐 successor。Client local state 是可并存的 current-authority/candidate-enrollment 两槽，replacement Ack 后崩溃由 candidate exact replay 收敛。Proto aggregate quality 只有 `EnrollmentEvidenceQuality::{MEDIUM,STRONG}`，`SyncSecret.password` 要求 present 且继承 Account password 值域；canonical proof/bundle/state message 字段集合变化必须切换 subprotocol。当前 Rust、migration 与历史 evidence 均尚未同步。
>
> **2026-08-24 修订（version / attachment / delivery cut）**：目标 exact subprotocol 为唯一 wire-generation 权威 `natsume.control.v1`，Challenge/Proof 不含 numeric version；proof transcript 使用实际 subprotocol。每个非终态 Enrollment transaction 至多一个 current Handshake attachment，新 exact replay 原子替换它，但 replacement attachment 与旧 Active lease 并存至 exact Ack activation。目标 `commands.state` 为 `queued` / `in_flight` / `succeeded` / `failed` / `outcome_unknown`，socket write 前 durable cut；ErrorCode wire token 采用受限开放语法并对 unknown 保持 opaque。下方旧 token、八态 DB、deadline、journal 与旧 error code 均是现有 Rust/DB 历史 evidence，不得解释为目标协议。
...
- **`sync_secret` 的秘密边界**：payload schema v1 不含 password（秘密不得进入 `frozen_payload_json`/DB，[契约](../contracts.md) §6）；wire required `SecretBytes` 由渲染时从 vault 注入，必须是 Account vault exact、非空、UTF-8、无 control character、最多 512-byte password，该注入属 Phase 5 `SYNC_SECRET` 语义。Phase 4 接受并持久化 `sync_secret` Command，但 dispatcher 对其不渲染不投递（typed 内部 hold，零 wire 效果），登记为 Phase 5 接线 hook。
