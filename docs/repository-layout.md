...
`device-protocol` 已按预发布 breaking change 原位拥有六文件单一 `natsume.device.control` package、定向 handshake/Active envelopes（Server Challenge|Bundle|Activated|SessionReady；Client Proof|Ack|Ready）、durable `enrollment_id`、结构化 Proof purpose 与 proof transcript；**无 ControlKeyId**（Server natural key 是 `public_key`，daemon manifest pins hex(public_key)）。无 `ClientInit`、无 `ControlEnvelope`、无 Hello、无 `ProofIntent` enum。不存在 `control_v2`、第二 descriptor 或兼容 generated module。Daemon `control` 当前只有 dormant PKCS#8 key/manifest foundation且尚未接入网络；目标 manifest 必须表达 Prepared / BundleInstalled / Active durable phase。后续 `http/device_control` 拥有 PreAuth transport，`application/device/control` 拥有动态 registry/actor/lease，`db/device` 维持明确的 transaction adapter。
