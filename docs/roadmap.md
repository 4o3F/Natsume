...
> 状态：`ACTIVE-PLAN`
> 架构基线：Natsume V2 v2.8 决策集合（[ADR-0022](adr/0022-deployment-facts-and-trust-assumptions.md)–[0029](adr/0029-right-sizing-control-plane-machinery.md)）
> 当前阶段：Phase 0（收尾）
> 计划基线：26 周 + 2–4 周外部缓冲（约 6 个月开发窗口，3 人，ADR-0022 F7）
> 注意：日期和周数是计划，不是完成声明
...
1. 先冻结高风险边界，再构建业务功能。
2. Gate 缺证据即未通过；证据 = 指向 CI run / commit / artifact 的链接 + 一行结论（[ADR-0029](adr/0029-right-sizing-control-plane-machinery.md)），不接受"先通过后补证据"。
...
|---|---:|---|---|
| 0. Engineering Baseline（收尾） | W1–W2 | 可重现仓库、真实 CI、WSS/token 契约骨架、空壳 Deb、输入冻结、DOMjudge lab 启动 | G0 |
...
- **P0.4 Contract skeleton（v2.8 重定向）**：窗口门禁 Enrollment（token + gateway）、WSS envelope（一帧一消息）、Observed/CommandStatus、Local D-Bus、SQL migrations、machine-generated golden、`INV-CERT-01` 两段阶梯负向 contract test。**删除**：QUIC/framing 骨架、Device Identity CSR 契约、mTLS client verifier 骨架。
...
| 镜像晚到/大版本变化 | `G0-IN-001/004` 输入门禁 + 重验清单（ADR-0027） |
...
Server migration 与模块边界、Seat/account/credential metadata、Device 与 binding、Server vault、operator auth（admin/viewer）、AuditEvent（与领域事务同事务）、operator API/OpenAPI、Web navigation 与 auth shell（轮询）。
...
`seat,account,password` 固定完整 candidate、encrypted staging、Server 权威 redacted diff、单 pending candidate、双 CAS Import Commit、atomic unbind-and-replace、opaque preview token、Preparation Center。

**G2 覆盖**：malformed/duplicate/empty candidate 拒绝、first/no-op/material 区分、双 CAS 拒绝与重复提交安全失败、事务回滚、password 明文不进任何普通 surface、CSV → Server truth 且零自动 Command。

### Phase 3：Identity & Enrollment

固定配方 `machine-identity`（ADR-0025）、privileged raw collectors、identity file、identity-before-credentials、凭据文件（ADR-0026）、provisioning 窗口状态机、窗口内 Token + Gateway 联合签发（ADR-0021/0023）、替换语义与审计。

**G3 覆盖**：identity 决策表全路径、configured-disk copy fixture、窗口开/关负向、联合签发事务原子性、CSR SAN ignore、替换语义与旧连接异常审计、package upgrade 保留 identity/凭据。

### Phase 4：Control Channel & Command Runtime

WSS + Bearer token 认证、subprotocol 版本协商、401-before-decode、durable Command dispatcher、Device journal、receipt/status、Observed snapshot（变化 + 低频兜底）、retry/reconnect/idempotency。

**G4 覆盖**：frame size/version/unknown enum、token 吊销即断、duplicate delivery 与 crash recovery、stale/conflict 拒绝、断线重连收敛、ErrorCode 跨 transport 一致。

### Phase 5：State, Data Plane & Secrets

Target derivation（派生代际）、explicit `SYNC_STATE`、Caddy 配置渲染/validate/reload/LKG 回滚、BLOCKED/READY、fixed TLS upstream、xheaders `/login` 注入（ADR-0024）、explicit `SYNC_SECRET` 与凭据文件/配置重渲染、Drift 与 operator views。
...
**G6 覆盖**：当期镜像 capability 清单全项（ADR-0027）、中文 IME/HiDPI/focus denied、display lost 与 Agent crash、无 user unit、epoch race、lock/unlock 的 Caddy 调用数为 0、Home prepare/cleanup/fault/reboot 与连续多次重置。
...
| 风险 | 缓解 |
|---|---|
| ICPC 镜像晚到或大版本变化 | 输入门禁 + 镜像升级重验清单（ADR-0027） |
| DOMjudge xheaders/brotli/TLS 契约不符 | lab 提前至 P0 收尾；结论冻结后才建 Phase 5 |
| 异构硬件 placeholder 超预期 | fixture 先行 + 首次 provisioning 实地全量验证（ADR-0025） |
