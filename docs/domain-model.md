...
**2026-08-24 修订（control lease 与数值域；2026-08-25 lane 澄清）**：HWID-sharded `MachineActor` 内每个 non-revoked `DeviceLane(device_id)` 同时至多一个 current control lease；lease 在内存中携带 owner DeviceId，新认证连接用 16-byte UUIDv7 `session_id` 只替换同 lane 的旧 lease。SQLite INTEGER-backed credential revision 与 Home epoch 的 protobuf 有效域统一为 `1..=i64::MAX`。Gateway state/hash 是独立 evidence，只有 `READY + expected leaf hash` 才收敛。
...
**2026-08-25 修订（Client failure、durable order 与 revoked successor）**：Client 以统一 `ClientClose` best-effort 报告本连接无法安全继续，但它不改变 durable 领域状态。Client 本地 current authority 与 candidate Enrollment 是两个可并存槽位；candidate 在 replacement Ack 崩溃恢复中优先 exact replay。Command 使用 Device lane 内持久化 `enqueue_order` 恢复严格顺序；disable/revoke transaction 在驱逐前收口准确旧 lane 的非终态 Command。revoked 旧 Device 永不恢复；显式本地 reprovision 只能通过新的人工审核 Enrollment 创建新 `device_id` 与新 lane，旧历史保留。

Control-key history、人工审核的 durable Enrollment transaction、immutable CredentialBundle、EnrollmentActivated/Ready barrier 与动态 MachineActor/DeviceLane 只有在 atomic flag-day schema/application 同批接线后才替代当前模型。Target transaction state 是 `pending_review` / `awaiting_credential_ack` / `active` / `denied`，无 Enrollment expiry 或 activation deadline。Client 的 `current_authority: Option<ActiveManifest>` 与 `candidate_enrollment: Option<Prepared | BundleInstalled>` 正交；candidate connection 使用 Enrollment purpose，current-authority connection 使用 Resume purpose，Activated 本地提交原子替换前者并清除后者。届时删除 Token-era states/rows并收紧 transitional NULL，不维持双 authority。
