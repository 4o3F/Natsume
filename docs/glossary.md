...
| **Observed snapshot** | Device 报告的 slim typed 实际状态：`applied_hash`、`installed_binding_id`、`installed_credential_revision`、`credential_state`、`gateway_state`、`gateway_certificate_fingerprint`、`session_state`。无 `secret_state`、无 `STALE` variant。 |
...
| **Command** | 面向单台 Device 的显式意图，分 Converge（领域键幂等，Server 重推同一 payload）与 Oneshot（仅 live socket，离线丢弃）。`commands` current row 是 operator 审计；正确性不是 Client journal。 |
...
| **Oneshot command** | `lock_session`、`unlock_session`、`terminate_session`、`open_binding_prompt`：仅 live socket；离线丢弃；重连不重放。 |
...
| **BindingRequest** | Device 在 live Active socket 上发起的绑定尝试（`binding_request_id` + `seat_code`）。不是 `OPEN_BINDING_PROMPT` 的 Command reply。 |
| **BindingResult** | Server 对 `BindingRequest` 的应答（`binding_request_id`、`state`、`error_code`）。不携带 occupancy `binding_id`；Device 从随后的 `SYNC_STATE` / `SYNC_SECRET` 得知该 stamp。 |
...
| **Binding ID / `binding_id`** | `device_bindings.binding_id`：每次 bind 铸造的 canonical lowercase hyphenated UUIDv7 occupancy stamp，独立于 Seat，供 `SyncState` / `SyncSecret` / Observed 冻结。`BindingResult` 不携带该字段。unbind 删除该行；再次 bind 得到新 UUID。无 integer bump，无 `seats.binding_generation`，无 `revision_counters`。Import 不写入 Binding、不铸造该 ID。它不表示 Seat→Account mapping。不存在全局 binding-set clock。 |
...
| **Session epoch** | 本地 graphical session 身份（Agent lease / logind）。WSS Oneshot 命令不携带 `session_epoch` / `SessionTarget`；目标就是该 Device 当前 session。 |
| **Home epoch** | `HOME_RESET` 的 Converge 键。同 epoch 可重入，已完成则为 success/no-op；`HOME_EPOCH_STALE` 仅当 epoch < 已完成 epoch；重试不得 bump epoch。 |
| **Client 凭据文件** | Device 本地 service-user-owned 权限文件：Device Token、Seat 凭据、identity record、dormant `control-key-1.pk8` 与 control manifest（pins hex(public_key)，无 ControlKeyId）为 `0600 natsume:natsume`，Gateway key/leaf 与含凭据 Caddy 配置为 `0640 natsume:natsume-gateway`；全部原子写，无应用层加密（ADR-0032）。 |
...
| **Enrollment** | provisioning 窗口内的 server-auth HTTPS 注册流程。`enrollment_requests` 记录硬件、CSR/SPKI、client/protocol/source-IP、state、可选 resolution/resolved device/issuance audit 与 `created_at_unix_ms`；`device_tokens` 和 `gateway_certificates` 各自以唯一 Enrollment request 关联签发结果。重复注册为受审计的替换。operator list 的 HTTP `created_at` 仍为 RFC 3339 UTC。 |
