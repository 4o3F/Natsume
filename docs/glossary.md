...
2026-08-24：ADR-0038 目标新增统一人工审核 Enrollment 状态机和 `EnrollmentReviewStatus`，移除 transaction expiry 与 challenge ID；Handshake/Active 统一以 `ServerClose{error_code,ServerCloseAction}` 表达 Server 终止；每台 Device 至多一个 UUIDv7 current control lease；Command 双端由有界 single-consumer channel 严格串行；binding prompt/request 用每 active session 单 in-flight 关联；credential convergence 使用完整 `{binding_id,account_id,credential_revision}`；Home convergence 增加 `completed_home_epoch`，Gateway observation 明确为完整 leaf DER SHA-256。
...
| **Observed snapshot** | Device 报告的 slim typed 实际状态：`applied_hash`、`CredentialObservation{binding_id,account_id,credential_revision,state}`、`gateway_state`、可选 `gateway_leaf_sha256`、`session_state`、可选 `completed_home_epoch`。Gateway state/hash 是独立 evidence，只有 `READY + expected leaf hash` 才收敛；其他组合保留为 drift/inconsistency。无 password、无 `secret_state`、无 `STALE` variant、无通用 Home state。ADR-0038 Active admission 中，SessionReady 后第一条 Active packet 必须是 Observed；旧数据库 row 只是 last-known evidence。 |
...
| **ServerClose（ADR-0038 目标）** | Server 在 Handshake 或 Active 中统一使用的终止包：非空 stable `error_code` 加非 `UNSPECIFIED` `ServerCloseAction`。`RETRY` 只进入 Client 统一退避，`STOP` 终止当前 local intent；协议无 Server retry delay。Client 只按 action 行为；每连接最多一次，随后无业务包并关闭。它不是 durable Enrollment 审核结果。 |
| **ClientProof（ADR-0038 目标）** | Device 对 Challenge 的 Ed25519 proof。`oneof purpose` 为 `EnrollmentAttempt` 或空 `ResumeSession`；前者必带 durable canonical UUIDv7 `enrollment_id`、candidate public key、exact CSR 与 aggregate evidence quality，后者不携带 Enrollment material。`daemon_version` 是当前运行 Daemon、`agent_version` 是已安装 Agent 的 canonical SemVer，供 Panel 消费但不参与 authority。无 `ProofIntent` enum。 |
...
| **EnrollmentReviewStatus（ADR-0038 目标）** | Server→Client 的 Enrollment 业务状态包：`PENDING_REVIEW` 表示 durable 等待人工审核且 `error_code` 为空，`DENIED` 携带非空 stable `error_code`。批准不新增 status variant；不可变 `CredentialBundle` 本身即批准结果。 |
...
| **SessionReady（ADR-0038 目标）** | 双端 durable Enrollment barrier 完成后的 connection-local control lease：`session_id` 是 Server-generated UUIDv7 的 16-byte RFC 9562 network-order encoding。Active envelope 只精确回显该值；lease 不持久化，也不是 Enrollment commit receipt 或公开 Identifier。发送后 Server actor 先进入 `AwaitingInitialObserved`，第一条 Active packet 必须是 Observed。 |
| **DeviceActor（ADR-0038 目标）** | 动态 registry 按需创建、进程内不淘汰的单 Device 排序点；Machine Hardware ID 只负责 shard routing，DeviceId 与 `public_key` 是 aliases。每个 actor 有一个有界 single-consumer mailbox 和至多一个 current control lease；新认证连接在 SessionReady 前原子替换旧 lease。无 ControlKeyId。 |
...
| **Credential revision** | `accounts.credential_revision`：某 Account 当前秘密的单调修订，有效 durable/wire 域为 `1..=i64::MAX`。任何已提交的 Import Commit 都无条件替换该 Account 的当前 ciphertext 并推进此修订，不做明文比较。每个 Account 只有其关联的 current vault ciphertext，不保留可寻址的旧密码版本。 |
...
| **Home epoch** | `HOME_RESET` 的 Server Target `home_epoch` 与 Device Observed 可选 `completed_home_epoch` 的出现值都在 `1..=i64::MAX`；后者是最近完整 durable 完成值。缺失/较小未收敛，相等收敛；超前或观测回退 fail closed。同 epoch 可重入，已完成则为 success/no-op；`HOME_EPOCH_STALE` 仅当命令 epoch < 本地已完成 epoch；重试不得 bump epoch。 |
