...
2026-08-24：ADR-0038 目标新增统一人工审核 Enrollment 状态机和 `EnrollmentReviewStatus`，移除 transaction expiry 与 challenge ID；Handshake/Active 统一以 `ServerClose` 表达 Server 终止；Command 双端由有界 single-consumer channel 严格串行；binding prompt/request 用每 active session 单 in-flight 关联；credential convergence 使用完整 `{binding_id,account_id,credential_revision}`；Home convergence 增加 `completed_home_epoch`，Gateway observation 明确为完整 leaf DER SHA-256。
...
| **Observed snapshot** | Device 报告的 slim typed 实际状态：`applied_hash`、`CredentialObservation{binding_id,account_id,credential_revision,state}`、`gateway_state`、可选 `gateway_leaf_sha256`、`session_state`、可选 `completed_home_epoch`。无 password、无 `secret_state`、无 `STALE` variant、无通用 Home state。ADR-0038 Active admission 中，SessionReady 后第一条 Active packet 必须是 Observed；旧数据库 row 只是 last-known evidence。 |
...
| **ServerClose（ADR-0038 目标）** | Server 在 Handshake 或 Active 中统一使用的终止包：stable `error_code` 加 required `retry` / `stop` action。Client 只按 action 决定是否自动重连；每连接最多一次，随后无业务包并关闭。它不是 durable Enrollment 审核结果。 |
...
| **Home epoch** | `HOME_RESET` 的 Server Target 是 `home_epoch`；Device Observed 可选 `completed_home_epoch` 是最近完整 durable 完成值。缺失/较小未收敛，相等收敛；超前或观测回退 fail closed。同 epoch 可重入，已完成则为 success/no-op；`HOME_EPOCH_STALE` 仅当命令 epoch < 本地已完成 epoch；重试不得 bump epoch。 |
