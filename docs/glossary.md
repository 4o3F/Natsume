...
| **Contest configuration revision** | `revision_counters.configuration_revision`；`0` 表示空 baseline；Seat 集合、Seat→Account mapping 或密码内容实际变化时递增；用作 import baseline CAS token。 |
| **Credential revision** | `accounts.credential_revision`：某 Account 当前秘密发生 material change 后的单调修订。每个 Account 只有其关联的 current vault ciphertext，不保留可寻址的旧密码版本。 |
...
| **Import Commit** | Operator 对 candidate 的显式二次确认动作；以双 CAS（`configuration_revision` + `binding_revision`）校验后原子应用。前者保护配置内容，后者保护 Binding 集合；material commit 含必要的 unbind-and-replace 与相应 revision 提升；no-op 仅 lineage/redacted audit。不自动产生 Device Command。 |
